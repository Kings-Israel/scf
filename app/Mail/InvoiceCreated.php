...
class InvoiceCreated extends Mailable
{
  ...
  {
    $this->template = EmailTemplate::where('name', 'Invoice Creation')->first();
  ...
  public function envelope()
  {
    $subject = '';
    $data = array();
    $data['seller_company'] = $this->invoice->company->name;
  ...
  public function content()
  {
    ...
    $invoices = '<tr><td>' . $this->invoice->invoice_number . '</td><td>' . $this->invoice->company->name . '</td><td>' . $this->invoice->invoice_total_amount . '</td><td>' . Carbon::parse($this->invoice->due_date)->format('d M Y') . '</td></tr>';
    $data['table'] =
      '<table>
    <thead>
    <tr>
    ...
    return new Content(
      markdown: 'content.email.invoice-created',
      with: [
