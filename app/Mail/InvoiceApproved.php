...
class InvoiceApproved extends Mailable
{
  ...
  {
    $this->template = EmailTemplate::where('name', 'Invoice Approval')->first();
  ...
  public function envelope()
  {
    $subject = '';
    $data = array();
    $data['buyer_company'] = $this->invoice->buyer ? $this->invoice->buyer->name : $this->invoice->program->anchor->name;
  ...
  public function content()
  {
    ...
    $data['invoice_amount'] = number_format($this->invoice->total + $this->invoice->total_invoice_taxes - $this->invoice->total_invoice_fees - $this->invoice->total_invoice_discount);
    $data['pi_amount'] = number_format($this->invoice->total + $this->invoice->total_invoice_taxes - $this->invoice->total_invoice_fees - $this->invoice->total_invoice_discount);
    ...
    return new Content(
      markdown: 'content.email.invoice-approved',
      with: [
