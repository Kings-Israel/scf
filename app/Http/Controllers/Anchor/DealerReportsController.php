...
class DealerReportsController extends Controller
{
  public function index()
  {
    ...
    $programs = Program::whereHas('programType', fn($query) => $query->where('name', Program::DEALER_FINANCING))
    ...
    return view('content.anchor.dealer.invoices-reports', [
  ...
  {
    return view('content.anchor.dealer.reports.all-invoices');
  }

  public function programsReportView()
  {
    return view('content.anchor.dealer.reports.dealer-programs');
  }

  public function paymentsReportView()
  {
    return view('content.anchor.dealer.reports.payments');
  }
  ...
  public function allInvoicesReport(Request $request)
  {
    ...
    $invoices = Invoice::dealerFinancing()
      ...
      ->where(function ($query) use ($current_company) {
        $query
          ->where('company_id', $current_company->company_id)
          ->orWhereHas('program', function ($query) use ($current_company) {
  ...
  public function factoringPaymentsReport(Request $request)
  {
    ...
    $payments = Invoice::factoringDealer()
      ->where(function ($query) use ($current_company) {
        $query->where('company_id', $current_company->company_id);
      })
    ...
    switch ($type) {
      case 'invoice-analysis':
        ...
        foreach ($invoices as $key => $payment) {
          $data[$key]['Buyer'] = $payment->company->name;
      ...
      case 'all-invoices':
        ...
        foreach ($invoices as $key => $invoice) {
          $data[$key]['Buyer'] = $invoice->company->name;
      ...
      case 'discountings':
        ...
        $headers = ['Buyer', 'Invoice Number', 'Amount', 'Disbursement Date', 'Transaction Ref'];
        $data = [];
      ...
      case 'factoring-program-report':
        ...
        foreach ($programs as $key => $program) {
          ...
          $data[$key]['Buyer'] = $program->company->name;
      ...
      case 'factoring-dealer-program-report':
        ...
        $programs = OdAccountsResource::collection(
          ProgramVendorConfiguration::with('program')
            ->whereHas('program', function ($query) use ($current_company) {
              $query
                ->whereHas('anchor', function ($query) use ($current_company) {
                  $query->where('companies.id', $current_company->company_id);
        ...
        foreach ($programs as $key => $program) {
          $data[$key]['Loan Account No'] = $program->payment_account_number;
          $data[$key]['Buyer'] = $program->company_name;
          $data[$key]['Financing Limit'] = number_format($program->sanctioned_limit, 2);
          $data[$key]['Utilized Limit'] = number_format($program->utilized, 2);
          $data[$key]['Pipeline Amount'] = number_format($program->pipeline, 2);
          $data[$key]['Available Limit'] = number_format(
      ...
      case 'factoring-payments-report':
        ...
        $payments = Invoice::dealerFinancing()
          ->whereHas('program', function ($query) use ($company) {
            $query->whereHas('anchor', function ($query) use ($company) {
              $query->where('company.id', $company->id);
            });
          })
          ->where(function ($query) use ($current_company) {
            $query->where('company_id', $current_company->company_id);
  ...
  public function exportPdf(Request $request, $type)
  {
    ...
    switch ($type) {
      ...
      case 'maturing-invoices':
        ...
        break;
      case 'vendor-analysis':
        $headers = ['Buyer', 'Limit', 'Utilized Limit', 'Available Limit', 'Pipeline Amount'];
        $data = [];
        $current_company = auth()
          ->user()
          ->activeAnchorDealerCompany()
        ...
        foreach ($company->programs as $program) {
          if ($program->programType->name == Program::DEALER_FINANCING) {
            $program_vendors = $program->getDealers();
            ...
            foreach ($program_vendors as $key => $vendor) {
              ...
              $vendor_configuration = ProgramVendorConfiguration::where('program_id', $program->id)
                ...
                ->first();
              $utilized_amount = $vendor->utilizedAmount($program);
              $pipeline_amount = $vendor->pipelineAmount($program);
              $data[$key]['Buyer'] = $vendor->name;
              $data[$key]['Sanctioned Limit'] = number_format($vendor_configuration->sanctioned_limit, 2);
              $data[$key]['Utilized Amount'] = number_format($utilized_amount, 2);
              $data[$key]['Pipeline Amount'] = number_format($pipeline_amount);
              $data[$key]['Available Amount'] = number_format(
                $vendor_configuration->sanctioned_limit - $utilized_amount - $pipeline_amount,
                2
              );
            }
          }
      ...
      case 'factoring-all-invoices-report':
        ...
        $invoices = Invoice::dealerFinancing()
          ...
          ->where(function ($query) use ($current_company) {
            $query->whereHas('program', function ($query) use ($current_company) {
              $query->whereHas('anchor', function ($query) use ($current_company) {
                $query->where('companies.id', $current_company->company_id);
              });
            });
        ...
        $pdf = Pdf::loadView('pdf.report', [
          'reportTitle' => 'Dealer All Invoices Report',
          'headers' => $headers,
          'data' => $data,
        ])->setPaper('a4', 'landscape');

        return $pdf->download('Dealer_All_Invoices_Report_' . $date . '.pdf');
        break;
      case 'factoring-program-report':
        $current_company = auth()
          ->user()
