...
class InvoicesImport implements ToCollection, WithHeadingRow, SkipsEmptyRows, SkipsOnFailure, WithEvents, WithMapping
{
  ...
  public function map($row): array
  {
    if (
      ...
      !array_key_exists('loanod_account_no', $row) ||
    ...
    return [
      ...
      'payment_od_account' => $row['loanod_account_no'],
      'pay_date' =>
      ...
      try {
        if (
            ...
            if ($invoice->program->programType->name == Program::VENDOR_FINANCING) {
              if (
                ...
                if ($invoice_setting && $invoice_setting->maker_checker_creating_updating) {
                  // Requires another company user to approve
                  $invoice->approvals()->create([
                    'user_id' => auth()->id(),
                  ]);
            ...
            if ($this->company) {
              if (!$vendor_configuration) {
                ...
                $invoice_upload_report->pay_date = $row['pay_date']
                  ...
                  : null;
                $invoice_upload_report->attachments = $row['attachments'];
                $invoice_upload_report->tax_amount =
                  array_key_exists('tax_amount', $row->toArray()) && !empty($row['tax_amount'])
                    ? $row['tax_amount']
                    : 0;
                $invoice_upload_report->total_payable = $row['total_payable']
                  ? str_replace(',', '', str_replace('Ksh', '', $row['total_payable']))
                  : 0;
                $invoice_upload_report->credit_note_amount =
                  array_key_exists('credit_note_amount', $row->toArray()) && !empty($row['credit_note_amount'])
                    ? $row['credit_note_amount']
                    : 0;
                $invoice_upload_report->batch_id = $latest_batch_id ? $latest_batch_id->batch_id + 1 : 1;
          ...
          if ($this->company) {
            ...
            $invoice_upload_report->attachments = $row['attachments'];
            $invoice_upload_report->tax_amount =
              array_key_exists('tax_amount', $row->toArray()) && !empty($row['tax_amount']) ? $row['tax_amount'] : 0;
            $invoice_upload_report->total_payable = $row['total_payable']
              ? str_replace(',', '', str_replace('Ksh', '', $row['total_payable']))
              : 0;
            $invoice_upload_report->credit_note_amount =
            ...
            if (!$row['currency']) {
              $invoice_upload_report->description = 'The Currency is required';
            }
            ...
            if (!$row['payment_od_account']) {
              $invoice_upload_report->description = 'The Loan/OD Account is required';
            }
