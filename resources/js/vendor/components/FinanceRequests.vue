    ...
    <div class="card-header d-flex flex-column flex-md-row justify-content-between">
      ...
      <div class="d-flex justify-content-md-end mt-2 mt-md-auto">
        <div style="margin-right: 10px">
          <select class="form-select" aria-label="Pagination Select" v-model="per_page">
...
export default {
  ...
  setup() {
    ...
    const getFinanceRequests = async () => {
      await axios
        .get(base_url + 'finance-requests/data', {
          params: {
    ...
    const updateApprovalStatus = (request, status) => {
      ...
      axios
        .post(base_url + 'finance-requests/' + request + '/status/update', formData)
        .then(() => {
