<template>
  <div class="card">
    <div class="card-header d-flex justify-content-between">
      <h6>{{ $t('10 Payment Instructions eligible for Financing') }}</h6>
      <a :href="'cash/planner'" class="">{{ $t('View All') }} </a>
    </div>
    <div class="card-body">
            ...
            <tr v-for="invoice in invoices.data" :key="invoice.id">
              ...
              <td v-if="can_request" class="text-success">
                ...
                <button
                  ...
                  data-bs-toggle="modal"
                  data-bs-target="#dashboard-loading-modal"
                ></button>
                <div class="modal fade" id="dashboard-loading-modal" tabindex="-1" aria-hidden="true">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="row">
                          <div class="col-6">
                            ...
                            <div class="border-bottom d-flex justify-content-between">
                              <label for="nameWithTitle" class="form-label">{{ $t('Annual Discount Rate') }}(%)</label>
                          ...
                          <div class="col-6">
                            <flat-pickr
                              ...
                              :config="config"
                            />
                            <!-- <DatePicker v-once v-model="request_payment_date" inline class="w-full sm:w-[30rem]" @date-select="updateDiscount" :minDate="prime_min_date" :maxDate="prime_max_date" :disabledDates="prime_disabled_dates" :disabledDays="prime_disabled_days" /> -->
                          </div>
                        </div>
                  ...
                  <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <a
                          :href="'cash-planner/invoices/terms/vendor_financing/download'"
                          v-if="terms_text && terms_text.terms_conditions != ''"
                          target="_blank"
                        ...
                        <a
                          :href="'cash-planner/invoices/' + invoice.id + '/noa/download'"
                          target="_blank"
                          v-if="noa_text && noa_text.body != ''"
                          >{{ $t('Download NOA') }}</a
                        >
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <!-- <div class="form-group">
                <DatePicker v-once v-model="bulk_request_payment_date" inline class="w-full" @date-select="updateBulkDiscount" :minDate="prime_min_date" :maxDate="prime_bulk_max_date" :disabledDates="prime_disabled_dates" :disabledDays="prime_disabled_days" />
              </div> -->
            </div>
            <div class="modal-footer">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer d-flex justify-content-between">
              <small class="mt-4"
                >{{ $t('By Submitting, you agree to the') }}
                <a
                  :href="terms_text ? 'cash-planner/invoices/terms/vendor_financing/download' : '#'"
                  target="_blank"
                  class="text-primary"
                  style="cursor: pointer"
                  >{{ $t('Terms and Conditions') }}</a
      ...
      <upload-attachment v-if="invoice_details" :invoice-details="invoice_details" :key="uploading_file_key" />
    </div>
  </div>
</template>
...
export default {
  ...
  setup(props) {
    ...
    const uploading_file_key = ref(0)
        ...
        .then(response => {
          ...
          invoice_details.value.vendor_fee_details.forEach((fees, index) => {
            if (fees.type == 'percentage') {
              vendor_fees +=
                (fees.vendor_bearing_discount / 100) * ((fees.value / 100) * invoice_eligibility_amount.value);
              if (fees.vendor_bearing_discount > 0) {
                vendor_fee_details.value.push([fees.fee_name, fees.value + '%']);
              }
              if (fees.taxes) {
                vendor_tax_on_fees += (fees.taxes / 100) * ((fees.value / 100) * invoice_eligibility_amount.value);
              }
            }
            if (fees.type == 'amount') {
              vendor_fees += (fees.vendor_bearing_discount / 100) * fees.value;
              if (fees.vendor_bearing_discount > 0) {
            ...
            if (fees.type == 'per amount') {
              vendor_fees +=
                ...
                (Math.round(invoice_eligibility_amount.value / fees.per_amount) * fees.value);
              if (fees.vendor_bearing_discount > 0) {
                vendor_fee_details.value.push([
                  fees.fee_name,
                  fees.value + ' P.A ' + invoice_details.value.currency + ' ' + fees.per_amount
                ]);
              }
              if (fees.taxes) {
                vendor_tax_on_fees +=
                  (fees.taxes / 100) * (Math.round(invoice_eligibility_amount.value / fees.per_amount) * fees.value);
              }
            }
          });
          ...
          invoice_details.value.vendor_bank_details.forEach(bank_details => {
    ...
    const formatNoa = invoice => {
      let new_text = noa_text.value.body;
