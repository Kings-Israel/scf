          ...
          <div class="modal-dialog">
            <form class="modal-content" method="POST" @submit.prevent="addNewBaseRate">
              <div class="modal-header">
                <h5 class="modal-title" id="modalTopTitle">{{ $t('Add New NOA Template') }}</h5>
                <button
                  type="button"
                  class="btn-close"
                  id="close-add-modal"
              ...
              <div class="modal-body my-1">
                ...
                <h6 class="text-h6 mt-4">
                  {{ $t('Variables') }}
                  <i
                    class="tf-icons ti ti-info-circle ti-xs text-primary"
                    title="These can be added in the body within parenthesis. e.g. {date} or {buyerName}"
                  ></i>
                </h6>
                <div class="text-wrap">
                  date,buyerName,anchorName,company,anchorCompanyUniqueID,time,agreementDate,contract,anchorAccountName,anchorAccountNumber,anchorCustomerId,anchorBranch,anchorIFSCCode,anchorAddress,penalInterestRate,sellerName
                </div>
                <!-- <div class="form-group my-1">
          ...
          <tr class="text-nowrap" v-for="data in rates.data" :key="data.id">
            <td>
              ...
              <i
                ...
                title="NOA creation/updation awating approval"
              ></i>
            </td>
            <td>
              <span class="m_title">
              ...
              <div class="modal modal-top fade modal-lg" :id="'rate-' + data.id" tabindex="-1">
                <div class="modal-dialog">
                  <form class="modal-content" method="POST" @submit.prevent="updateBaseRate">
                    <div class="modal-header">
                      <h5 class="modal-title" id="modalTopTitle">{{ $t('Edit NOA Template') }}</h5>
                      <button
                        type="button"
                        class="btn-close"
              ...
              >
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 class="modal-title" id="modalTopTitle">{{ $t('Proposed Changes') }}</h5>
                      <button
                        type="button"
                        class="btn-close"
                    ...
                    <div class="modal-footer">
                      ...
                      <button
                        ...
                        @click="approveChange('App\\Models\\NoaTemplate', data.id, 'reject')"
                      >
                        {{ $t('Reject') }}
                      </button>
                      <button
                        ...
                        @click="approveChange('App\\Models\\NoaTemplate', data.id, 'approve')"
                      >
                        {{ $t('Approve') }}
                      </button>
                    </div>
...
export default {
  ...
  setup(props) {
    ...
    const getRates = async () => {
      await axios
        .get(base_url + props.bank + '/configurations/noa-templates/data', {
          params: {
    ...
    const addNewBaseRate = () => {
      axios
        .post(base_url + props.bank + '/configurations/noa-templates/store', {
          name: name.value,
        ...
        .then(() => {
          toast.success('NOA Template added successfully');
          name.value = '';
          body.value = '';
          status.value = '';
    ...
    const updateBaseRate = () => {
      axios
        .post(base_url + props.bank + '/configurations/noa-templates/update', {
          id: edit_id.value,
          name: name.value,
          status: status.value,
          product_type: product_type.value,
    ...
    const updateActiveStatus = (base_rate, status) => {
      axios
        .get(base_url + props.bank + `/configurations/noa-templates/${base_rate}/status/${status}/update`)
        .then(() => {
          getRates();
