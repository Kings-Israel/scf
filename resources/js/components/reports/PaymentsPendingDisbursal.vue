    ...
    <div class="card-body p-0">
        ...
        <div class="d-flex flex-wrap gap-1">
          ...
          <div class="">
            ...
            <input
              ...
              name="payments-pending-disbursal-report-requested-payment-daterange"
              v-on:keyup.enter="filter"
            />
          </div>
          <div class="">
      ...
      <div class="table-responsive">
        <table class="table">
          <thead>
            <tr class="">
              <th class="">{{ $t('System ID') }}.</th>
              <th class="">{{ $t('Invoice No') }}.</th>
              <th class="">{{ $t('Anchor') }}</th>
              <th class="">{{ $t('Vendor') }}</th>
              <th class="">{{ $t('Request Date') }}</th>
          ...
          <tbody class="table-border-bottom-0">
            ...
            <tr v-for="transaction in transactions.data" :key="transaction" class="text-nowrap">
              ...
              <td v-else class="">-</td>
              <td>
                {{ transaction.cbs_transactions.length > 0 ? transaction.cbs_transactions[0].debit_from_account : '-' }}
              </td>
              <td
                class="text-primary text-decoration-underline"
                data-bs-toggle="modal"
                :data-bs-target="'#payment-accounts-' + transaction.id"
                style="cursor: pointer"
              >
                {{ transaction.payment_accounts.length }} Account(s)
              </td>
              <td>
                <button
                  class="d-none"
                ...
                />
                <div class="modal fade" :id="'payment-accounts-' + transaction.id" tabindex="-1" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h5 class="modal-title" id="modalCenterTitle">Accounts</h5>
                      ...
                      <div class="modal-body">
                        <div class="table-responsive">
                          <table class="table">
                            ...
                            <tbody>
                              <tr v-for="account in transaction.payment_accounts" :key="account.id">
                                <td>{{ account.account }}</td>
                                <td class="text-success">{{ new Intl.NumberFormat().format(account.amount) }}</td>
                                <td>
                                  <span v-if="account.type == 'vendor_account'">Vendor's/Seller's A/C</span>
                                  <span v-if="account.type == 'discount'">Bank Discount A/C</span>
                                  <span v-if="account.type == 'program_fees'">Program Fees (Bank A/C)</span>
                                  <span v-if="account.type == 'program_fees_taxes'">Program Fees Taxes (Bank A/C)</span>
                                  <span v-if="account.type == 'tax_on_discount'">Tax on Discount (Bank A/C)</span>
                                </td>
                              </tr>
                            </tbody>
                          </table>
...
export default {
  ...
  setup(props) {
    ...
    const program_type_search = ref('vendor_financing_receivable');
    const requested_disbursement_date = ref('');
      ...
      await axios
        .get(base_url + props.bank + '/reports/data', {
          params: {
            ...
            type: 'loans-pending-disbursal-report'
          }
        })
        .then(response => {
      ...
      await axios
        .get(base_url + props.bank + '/reports/data', {
          params: {
            ...
            program_type: program_type_search.value,
            requested_disbursement_date: requested_disbursement_date.value,
            from_requested_disbursement_date: from_requested_disbursement_date.value,
            to_requested_disbursement_date: to_requested_disbursement_date.value,
            type: 'loans-pending-disbursal-report',
            sort_by: sort_by.value
    ...
    const refresh = async () => {
      ...
      vendor_search.value = '';
      requested_disbursement_date.value = '';
      program_type_search.value = 'vendor_financing_receivable';
      sort_by.value = '';
    ...
    const downloadReport = () => {
      ...
      axios
        ...
        .then(response => {
          ...
          fileLink.setAttribute('download', `Loans_pending_disbursal_report_${moment().format('Do_MMM_YYYY')}.csv`);
    ...
    const downloadPdf = () => {
      ...
      axios
        ...
        .then(response => {
          ...
          fileLink.href = fileURL;
          fileLink.setAttribute('download', `Loans_pending_disbursal_Report_${moment().format('Do_MMM_YYYY')}.pdf`);
          document.body.appendChild(fileLink);
