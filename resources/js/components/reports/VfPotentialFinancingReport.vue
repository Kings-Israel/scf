  ...
  <div class="card p-2" id="companies">
    <div class="d-flex flex-column flex-md-row justify-content-between">
      <div class="d-flex flex-wrap gap-1">
        ...
        <div class="">
          <label class="form-label">{{ $t('Invoice Date') }}</label>
          <input
            class="form-control form-search"
            type="text"
            id="invoice_date_search"
            name="vf-potential-financing-report-invoice-daterange"
            v-on:keyup.enter="filter"
          />
        </div>
        <div class="">
          ...
          <input
            ...
            type="text"
            id="due_date_search"
            name="vf-potential-financing-report-due-daterange"
            v-on:keyup.enter="filter"
          />
        </div>
        <div class="">
...
export default {
  name: 'VFPotentialFinancingReport',
  components: {
    Pagination,
    DownloadButtons,
    InvoiceDetails,
  ...
  setup(props) {
    ...
    const from_invoice_date = ref('');
    const to_invoice_date = ref('');
    const due_date = ref('');
    const from_due_date = ref('');
    const to_due_date = ref('');
    const sort_by = ref('');
      ...
      await axios
        .get(base_url + props.bank + '/reports/data', {
          params: {
            ...
            type: 'vf-potential-financing-report'
          }
        })
        .then(response => {
      ...
      await axios
        .get(base_url + props.bank + '/reports/data', {
          params: {
            ...
            from_invoice_date: from_invoice_date.value,
            from_invoice_date: to_invoice_date.value,
            due_date: due_date.value,
            from_due_date: from_due_date.value,
            to_due_date: to_due_date.value,
    ...
    const filter = async () => {
      ...
      if (due_date_search.value) {
        from_due_date.value = moment(due_date_search.value.split(' - ')[0]).format('YYYY-MM-DD');
        to_due_date.value = moment(due_date_search.value.split(' - ')[1]).format('YYYY-MM-DD');
      }
      invoice_date_search.value = $('#invoice_date_search').val();
      if (invoice_date_search.value) {
        from_invoice_date.value = moment(invoice_date_search.value.split(' - ')[0]).format('YYYY-MM-DD');
        to_invoice_date.value = moment(invoice_date_search.value.split(' - ')[1]).format('YYYY-MM-DD');
      }
    ...
    const refresh = async () => {
      ...
      invoice_date.value = '';
      from_invoice_date.value = '';
      to_invoice_date.value = '';
      due_date.value = '';
      from_due_date.value = '';
      to_due_date.value = '';
      sort_by.value = '';
