      ...
      <div class="d-flex flex-wrap gap-1">
        ...
        <div class="">
          ...
          <input
            ...
            class="form-control form-search"
            name="factoring-income-report-payment-daterange"
        ...
        <div class="">
          ...
          <select class="form-select form-search" id="exampleFormControlSelect1">
      ...
      <table class="table">
        <thead>
          <tr class="">
            ...
            <th>{{ $t('Total Posted Penal Discount') }}</th>
            <th>{{ $t('Status') }}</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr v-for="report in report_data.data" :key="report.id">
            ...
            <td>
              <span
                class="badge bg-label-danger m_title"
                v-if="report.financing_status == 'disbursed' && moment().isAfter(report.due_date)"
                >Overdue</span
              >
              <span class="badge bg-label-success m_title" v-else>{{ report.financing_status }}</span>
            </td>
            <td>
              <button
                class="d-none"
...
export default {
  ...
  setup(props) {
    ...
    const downloadReport = () => {
      ...
      axios
        ...
        .then(response => {
          ...
          fileLink.setAttribute('download', `Factoring_income_report_${moment().format('Do_MMM_YYYY')}.csv`);
    ...
    const downloadPdf = () => {
      ...
      axios
        ...
        .then(response => {
          ...
          fileLink.href = fileURL;
          fileLink.setAttribute('download', `Factoring_income_Report_${moment().format('Do_MMM_YYYY')}.pdf`);
          document.body.appendChild(fileLink);
    ...
    return {
      ...
      invoice_number,
      payment_date,
      from_payment_date_search,
      to_payment_date_search,
      dpd,
      status,
      sort_by,
