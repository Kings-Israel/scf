    ...
    <div class="d-flex flex-column flex-md-row justify-content-between">
      <div class="d-flex flex-wrap gap-1">
        ...
        <div class="">
          ...
          <input class="form-control" type="date" value="" id="html5-date-input" v-model="uploaded_date_search" />
        </div>
        <div class="">
          <label for="Status" class="form-label">{{ $t('Status') }}</label>
          <select class="form-select" id="exampleFormControlSelect" v-model="status">
        ...
        <div class="table-clear-btn">
          <button class="btn btn-primary btn-sm" @click="clearSearch"><i class="ti ti-refresh"></i></button>
        </div>
      </div>
    ...
    <div class="table-responsive p-2">
      <table class="table dt-invoices">
        ...
        <tbody>
          ...
          <tr class="text-nowrap" v-show="invoices.data.length > 0" v-for="invoice in invoices.data" :key="invoice.id">
            <td>
              <span>{{ invoice.invoice_number }}</span>
            </td>
            <td>
              <span v-if="invoice.upload_status == 'Successful'" class="badge bg-label-success me-1">
                {{ invoice.status }}
              </span>
              <span v-if="invoice.upload_status == 'Failed'" class="badge bg-label-danger me-1">
                {{ invoice.status }}
              </span>
            </td>
            <td class="">
...
export default {
  ...
  setup(props, context) {
    ...
    const getInvoices = async () => {
      await axios
        .get(base_url + props.bank + '/requests/uploaded/data', {
          params: {
    ...
    const exportInvoices = () => {
      axios
        .get(base_url + props.bank + '/requests/uploaded/data/export', {
        ...
        .then(response => {
          ...
          fileLink.setAttribute('download', `Uploaded_Payment_Requests_${moment().format('Do_MMM_YYYY')}.xlsx`);
