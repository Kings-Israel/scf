<template>
  <div class="card p-1">
    <div class="d-flex justify-content-between">
      <h5 class="card-header px-1">{{ $t('Latest Vendor Financing Requests') }}</h5>
      <a class="card-header text-primary text-decoration-underline" :href="bank_url + '/requests/reverse-factoring'">{{
        $t('View All')
      }}</a>
    </div>
                  ...
                  <div class="modal-content">
                    <div class="modal-header">
                      ...
                      <div class="d-flex gap-1">
                        ...
                        >
                        <span class="mx-1" v-if="payment_request.user_can_approve && !payment_request.user_has_approved"
                          ><button
                        ...
                        <div>
                          <a
                            :href="bank_url + '/payment-request/' + payment_request.id + '/download'"
                            class="btn btn-primary"
                    ...
                    <div class="modal-body">
                      <div class="row mb-1">
                        ...
                        <div class="col-6">
                          <a
                            :href="bank_url + '/companies/' + payment_request.company_id + '/details'"
                        ...
                        <div class="col-6">
                          <a
                            :href="bank_url + '/companies/' + payment_request.anchor_id + '/details'"
                            class="fw-bold my-auto"
                            >{{ payment_request.anchor_name }}</a
                          >
                        </div>
                    ...
                    <form @submit.prevent="updateStatus(payment_request, 'approved')" method="post">
                      <div class="modal-body">
                        <h5>
                          {{
                            $t(
                              'Are you sure you want to approve the Vendor Financing request with the reference number, '
                            )
                          }}
                          {{ payment_request.reference_number }}?
                        </h5>
    ...
    const getRequests = async () => {
      await axios
        .get(
          base_url +
            props.bank +
            '/requests/vendor-financing-requests/data?per_page=' +
            per_page.value +
            '&type=dashboard'
        )
        .then(response => {
