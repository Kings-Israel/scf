          ...
          <div class="row border rounded border-primary mx-2 py-2">
            ...
            <div class="col-md-3 mt-2">
              <div class="form-check form-switch">
                ...
                <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked" :checked="auto_request_financing" v-model="auto_request_financing">
    ...
    <form @submit.prevent="updateInvoiceSettings" method="post">
      ...
      <div class="row px-3 mb-2">
        <div class="col-md-6 d-flex flex-column">
          <label for="" class="col-form-label">{{ $t('PO Description') }}</label>
          <small>{{ $t('Will be shown in PO as description') }}</small>
        </div>
      ...
      <div class="row px-3 mb-2 mt-4">
        <div class="col-md-6 d-flex flex-column">
          <label for="html5-text-input" class="col-form-label">{{ $t('PO Footer') }}</label>
          <small>{{ $t('Will be shown in PO footer') }}</small>
        </div>
        <div class="col-md-6">
      ...
      <div class="d-flex my-2 mx-3 d-flex justify-content-end">
        <button class="btn btn-primary" type="submit" v-if="can_submit_lower && can_edit == 1">
          {{ $t('Submit') }}
        </button>
      </div>
    </form>
...
export default {
  ...
  setup(props, context) {
    ...
    const getPurchaseOrderSettings = () => {
      axios.get('purchase-order-settings')
        .then(response => {
    ...
    const updateInvoiceSettings = () => {
      ...
      axios.post('purchase-order-settings/update', formData)
      .then((response) => {
    ...
    const updateStatus = (status) => {
      axios.get('purchase-order-settings/status/'+status+'/update')
      .then(response => {
        getPurchaseOrderSettings()
        invoice_logo.value = null
        toast.success(response.data.message)
    ...
    watch([maker_checker, auto_request_financing, invoice_logo, invoice_content, invoice_footer], ([maker_checker, auto_request_financing, invoice_logo, invoice_content, invoice_footer]) => {
