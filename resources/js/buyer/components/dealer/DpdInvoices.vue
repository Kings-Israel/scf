      ...
      <div class="d-flex justify-content-between">
        <div class="d-flex flex-wrap gap-1">
          <div class="">
            ...
            <input type="text" class="form-control form-search" placeholder="Invoice No" v-model="invoice_number" v-on:keyup.enter="filter" aria-describedby="defaultFormControlHelp" />
          </div>
          <div class="">
            <label for="Search" class="form-label">{{ $t('Anchor') }}</label>
            <input type="text" class="form-control form-search" placeholder="Anchor" v-model="anchor" v-on:keyup.enter="filter" aria-describedby="defaultFormControlHelp" />
          </div>
          <div class=" d-none">
            <label for="DPD Rage" class="form-label">{{ $t('DPD Range') }}</label>
            <select class="form-select  form-search">
          ...
          <tbody>
            ...
            <tr v-show="invoices.data.length > 0" v-for="invoice in invoices.data" :key="invoice.id">
              ...
              <td class="text-primary text-decoration-underline">
                <a href="javascript:;" @click="showInvoice(invoice.id, 'show-details-btn-'+invoice.id)">{{ invoice.invoice_number }}</a>
              </td>
              <td class="text-success">{{ invoice.currency }} <span>{{ new Intl.NumberFormat().format(invoice.invoice_total_amount) }}</span></td>
              <td class="">{{ invoice.disbursement_date ? moment(invoice.disbursement_date).format('DD MMM YYYY') : '-' }}</td>
              <td class="">{{ moment(invoice.due_date).format('DD MMM YYYY') }}</td>
              <td class="text-success">{{ invoice.currency }} <span>{{ new Intl.NumberFormat().format(invoice.disbursed_amount) }}</span></td>
              <td>
                <span>{{ invoice.days_past_due }}</span>
              </td>
              <td>
                <span class="">
                  {{ invoice.currency }}.{{ new Intl.NumberFormat().format(invoice.overdue_amount) }}
                </span>
              </td>
              <td>
                <button
...
export default {
  ...
  setup() {
    ...
    const getInvoices = async () => {
      await axios.get(base_url+'reports/dpd-invoices/data', {
        params: {
