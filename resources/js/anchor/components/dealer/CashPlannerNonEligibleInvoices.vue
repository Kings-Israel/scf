<template>
  <div class="card">
    <div class="card-header">
      <h6>{{ $t('Payments Instructions Not Eligible for Financing') }}</h6>
    </div>
    <div class="card-body">
      <pagination
        :from="invoices.from"
          ...
          <tbody class="table-border-bottom-0">
            ...
            <tr v-for="invoice in invoices.data" :key="invoice.id">
              <td
                ...
                :data-bs-target="'#cash-planner-invoice-' + invoice.id"
              >
                <a href="javascript:;" class="dropdown-item">{{ invoice.invoice_number }}</a>
              </td>
              <td class="">{{ invoice.anchor.name }}</td>
                ...
                <span>
                  {{
                    new Intl.NumberFormat().format(
                      (invoice.total + invoice.total_taxes - invoice.total_fees - invoice.total_discount).toFixed(2)
                    )
                  }}
                </span>
              </td>
              <td class="text-success">
                {{ invoice.currency }}
                <span>{{ new Intl.NumberFormat().format(invoice.eligible_for_finance.toFixed(2)) }}</span>
              </td>
              <td>
                <div
                  class="modal fade"
                  ...
                  <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        ...
                        <div class="d-flex">
                          <div class="d-flex" v-if="invoice_details && invoice_details.media.length > 0">
                            <a
  ...
  setup() {
    ...
    const showModal = invoice => {
      ...
      invoice_eligibility_amount.value =
        invoice.total + invoice.total_taxes - invoice.total_fees - invoice.total_discount;
    ...
    const getInvoices = async () => {
      await axios
        .get(base_url + 'factoring/cash-planner/invoices/non-eligible/data')
        .then(response => {
    ...
    const calculateRemittance = invoice => {
      ...
      let discount =
        legible_amount *
        (invoice.vendor_discount_details.total_roi / 100) *
        ((moment(invoice.due_date).diff(moment(), 'days') + 1) / 365);
    ...
    const requestFinance = async () => {
      ...
      await axios
        .post('cash-planner/invoices/request/send', {
    ...
    const requestFinanceForMultiple = async () => {
      ...
      await axios
        .post('cash-planner/invoices/request/multiple/send', formData)
        .then(response => {
