<template>
  <div class="card">
    <div class="card-header d-flex justify-content-between">
      ...
      <a :href="'factoring/planner'" class="">{{ $t('View All') }} </a>
    </div>
    <div class="card-body">
...
export default {
  ...
  setup(props) {
    ...
    const showModal = async (invoice, modal) => {
      ...
      await axios
        ...
        .then(response => {
          ...
          invoice_discount_amount.value =
            (invoice_details.value.vendor_discount_details.anchor_discount_bearing / invoice_business_spread.value) *
            invoice_discount_amount.value;
          ...
          invoice_details.value.vendor_fee_details.forEach((fees, index) => {
            if (fees.type == 'percentage') {
              vendor_fees +=
                (fees.anchor_bearing_discount / 100) * ((fees.value / 100) * invoice_eligibility_amount.value);
              if (fees.anchor_bearing_discount > 0) {
            ...
            if (fees.type == 'amount') {
              vendor_fees += (fees.anchor_bearing_discount / 100) * fees.value;
              if (fees.anchor_bearing_discount > 0) {
                vendor_fee_details.value.push([fees.fee_name, invoice_details.value.currency + ' ' + fees.value]);
              }
              if (fees.taxes) {
                vendor_tax_on_fees += (fees.taxes / 100) * ((fees.anchor_bearing_discount / 100) * fees.value);
              }
            }
    ...
    const formatNoa = invoice => {
      ...
      new_text = new_text.replace('{buyerName}', invoice.buyer.name);
