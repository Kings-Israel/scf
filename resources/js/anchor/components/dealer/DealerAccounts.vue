        ...
        <table class="table">
          ...
          <tbody class="table-border-bottom-0">
            ...
            <tr v-for="od_account in reports.data" :key="od_account.id">
              ...
              <td class="text-success text-nowrap">{{ new Intl.NumberFormat().format(od_account.sanctioned_limit) }}</td>
              <td class="text-success text-nowrap">{{ new Intl.NumberFormat().format(od_account.utilized_amount) }}</td>
              <td class="text-success text-nowrap">{{ new Intl.NumberFormat().format(od_account.pipeline_amount) }}</td>
              ...
              <td class="text-success text-nowrap">{{ new Intl.NumberFormat().format(od_account.available_amount) }}</td>
              <td class="">{{ od_account.overdue_days }}</td>
              <td>
...
export default {
  ...
  setup() {
    ...
    const getOdAccounts = async () => {
      await axios
        .get(base_url + 'factoring/invoices/od-accounts/data?per_page=' + per_page.value, {
          params: {
    ...
    watch([per_page], async ([per_page]) => {
      await axios
        .get(base_url + 'factoring/invoices/od-accounts/data', {
          params: {
            per_page: per_page,
            payment_account_number: payment_account_number_search.value,
            dealer: dealer_search.value
    ...
    const refresh = async () => {
      ...
      dealer_search.value = '';
      await axios.get(base_url + 'factoring/invoices/od-accounts/data?per_page=' + per_page.value).then(response => {
        reports.value = response.data.data;
