  ...
  <div class="p-2">
    <div class="d-flex justify-content-between">
      <div class="d-flex flex-wrap gap-1">
        ...
        <div class="">
          <label class="form-label" for="">{{ $t('Date') }} ({{ $t('Invoice Date') }})</label>
          <input
            type="text"
            id="invoice_date_search"
            class="form-control form-search"
            name="daterange"
            placeholder="Select Dates"
            autocomplete="off"
        ...
        <div class="">
          ...
          <select class="form-select form-search" v-model="sort_by">
            ...
            <option value="buyer_asc">{{ $t('Buyer. (ASC)') }}</option>
            <option value="buyer_desc">{{ $t('Buyer. (DESC)') }}</option>
          </select>
        </div>
        <div class="">
        ...
        <div class="table-clear-btn">
          <button class="btn btn-primary btn-md" @click="clearSearch">
                  ...
                  <table class="table">
                    ...
                    <tbody v-if="!invoice_fees_processing">
                      <tr v-for="invoice in invoices.data" :key="invoice.id">
                        ...
                        <td v-if="selected_invoices.includes(invoice.id)">
                          <span :id="'invoice-amount-' + invoice.id">
                            {{
                            ...
                            <div v-for="(tax_details, id) in invoice_tax" :key="id">
                              <div v-if="id == invoice.id">
                                <input
                                  ...
                                  step=".01"
                                  min="0"
                                  :id="'credit-note-amount-' + invoice.id"
                                  @input="calculatePayable(invoice.id)"
                                />
                              </div>
                            </div>
                          ...
                          <div v-for="(invoice_tax, key) in selected_invoices_taxes" :key="key">
                            <div v-for="(tax_details, id) in invoice_tax" :key="id">
                              <div v-if="id == invoice.id">
                                <span
                                  class="form-control"
                                  :id="'invoice-total-payable-' + invoice.id"
        ...
        <button class="d-none" id="show-bulk-reject-modal" data-bs-toggle="modal" data-bs-target="#bulk-invoice-reject">
          {{ $t('Reject') }}
        </button>
      ...
      <div class="d-flex justify-content-end gap-1 mt-auto">
        ...
        <button type="button" class="btn btn-primary" style="height: fit-content" @click="exportInvoices">
      ...
      <table class="table dt-invoices">
        ...
        <tbody>
          ...
          <tr v-show="invoices.data.length > 0" v-for="invoice in invoices.data" :key="invoice.id">
            ...
            <td>{{ invoice.buyer ? invoice.buyer : invoice.company }}</td>
            ...
            <td class="text-success text-nowrap">
              {{ invoice.currency }} <span>{{ new Intl.NumberFormat().format(invoice.discount) }}</span>
            </td>
            ...
            <td class="text-success">{{ new Intl.NumberFormat().format(invoice.disbursed_amount) }}</td>
            <td class="">
              <div class="d-inline-block">
                ...
                <ul class="dropdown-menu dropdown-menu-end">
                  ...
                  <li
                    v-if="!invoice.buyer"
                    style="cursor: pointer"
                    @click="showInvoice(invoice.id, 'show-approvals-btn-' + invoice.id)"
                  >
                    <span class="dropdown-item badge bg-label-info">{{ $t('Checker Approvals') }}</span>
                  </li>
                  <li
                    v-if="!invoice.buyer && invoice.authorization_group"
                    style="cursor: pointer"
            ...
            <td>
              ...
              <div
                class="modal fade"
                v-if="invoice_details"
                :id="'invoice-approval-' + invoice_details.id"
                tabindex="-1"
                aria-hidden="true"
              >
                <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      ...
                      <button
                        type="button"
                        :id="'close-modal-' + invoice_details.id"
                        class="btn-close"
                        data-bs-dismiss="modal"
                        aria-label="Close"
                      ></button>
                    </div>
                    <form @submit.prevent="approveFees(invoice_details)" :id="'view-fees-' + invoice_details.id">
                      <div class="modal-body">
                        ...
                        <div v-else>
                          <div class="d-flex justify-content-between">
                            <div>
                              <span>{{ $t('Fees not set for invoice ' + invoice_details.invoice_number) }}</span>
                            </div>
                            <!-- <button
                              class="btn btn-primary btn-sm"
                              :id="'edit-fees-btn-' + invoice_details.id"
                      ...
                      <div class="modal-footer">
                        <button class="btn btn-primary" type="submit">
                          {{ invoice_details.invoice_fees.length ? $t('Approve Fees') : $t('Approve Invoice') }}
                        </button>
                      </div>
                    </form>
                  </div>
                      ...
                      <table class="table">
                        ...
                        <tbody>
                          <tr v-for="user in users" :key="user.id">
                            <template v-if="userHasApproved(invoice, user)">
                              <td class="">
                                {{ user.name }}
                              </td>
                              <td>
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      <div v-for="user in invoice_details.authorization_group.authorization_users" :key="user.id">
                        <div v-if="userHasApproved(invoice, user.user)" class="d-flex">
...
export default {
  ...
  setup(props, context) {
    ...
    const from_invoice_date_search = ref('');
    const to_invoice_date_search = ref('');
    ...
    const editFees = invoice => {
      document.getElementById('edit-fees-' + invoice.id).classList.remove('d-none');
      document.getElementById('view-fees-' + invoice.id).classList.add('d-none');
    };

    const getInvoices = async () => {
      await axios
        .get(base_url + 'dealer/invoices/data', {
          params: {
            ...
            from_invoice_date: from_invoice_date_search.value,
            to_invoice_date: to_invoice_date_search.value,
    ...
    const showPurchaseOrder = async (purchase_order, modal) => {
      await axios
        .get(base_url + 'dealer/purchase-orders/' + purchase_order + '/show')
        .then(response => {
    ...
    const submitApproval = async invoice => {
      ...
      await axios
        .post(`${base_url}dealer/invoices/${invoice.id}/approve`, formData)
        .then(() => {
          getInvoices();
    ...
    const approveFees = async invoice => {
      await axios
        .get(`${base_url}dealer/invoices/${invoice.id}/fees/approve`)
        .then(() => {
          getInvoices();
          toast.success('Invoice updated successfully');
    ...
    const requestApproval = async (invoice, status) => {
      ...
      await axios
        .post(base_url + 'dealer/invoices/' + invoice.id + '/request/send', formData)
        .then(() => {
          toast.success('Invoice updated successfully');
          getInvoices();
        })
    ...
    const selectAll = () => {
      ...
      if (selected_invoices.value.length > 0) {
        ...
        axios.post(base_url + 'dealer/invoices/bulk/details?type=Factoring', formData).then(response => {
    ...
    const submitBulkApproval = () => {
      ...
      axios
        .post(base_url + 'dealer/invoices/bulk/update/approve?type=Factoring', formData)
        .then(res => {
          getInvoices();
          toast.success(res.data.message);
          document.getElementById('fees-close-modal').click();
          for (const item in invoice_taxes.value) {
            invoice_taxes.value[item] = '';
          }
          selected_invoices.value = [];
    ...
    const submitBulkRejection = () => {
      ...
      axios
        .post(base_url + 'dealer/invoices/bulk/update/reject?type=Factoring', formData)
        .then(() => {
          getInvoices();
    ...
    const submitRejection = id => {
      ...
      axios
        .post(base_url + 'dealer/invoices/' + id + '/reject', formData)
        .then(() => {
          getInvoices();
          toast.success('Invoices updated successfully');
    ...
    const clearSearch = async () => {
      ...
      date_search.value = '';
      invoice_date_search.value = '';
      sort_by.value = '';
      $('#date_search').val('');
      $('#invoice_date_search').val('');
      $('#invoice-status-select').val('');
      $('#invoice-status-select').trigger('change');
    ...
    const filter = async () => {
      ...
      invoice_date_search.value = $('#invoice_date_search').val();
      ...
      if (invoice_date_search.value) {
        from_invoice_date_search.value = moment(invoice_date_search.value.split(' - ')[0]).format('YYYY-MM-DD');
        to_invoice_date_search.value = moment(invoice_date_search.value.split(' - ')[1]).format('YYYY-MM-DD');
      }
    ...
    const calculatePayable = invoice_id => {
      let invoice_total = $('#invoice-amount-' + invoice_id)
        .text()
        .replaceAll(',', '');
      let withholding_tax = $('#withholding-tax-' + invoice_id).val();
      let withholding_vat = $('#withholding-vat-' + invoice_id).val();
      let credit_note_amount = $('#credit-note-amount-' + invoice_id).val();
      ...
      $('#invoice-total-payable-' + invoice_id).text(new Intl.NumberFormat().format(payable_amount));
    };
    ...
    return {
      ...
      from_date_search,
      to_date_search,
      from_invoice_date_search,
      to_invoice_date_search,
      sort_by,
