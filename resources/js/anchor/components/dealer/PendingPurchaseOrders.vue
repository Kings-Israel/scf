...
export default {
  ...
  setup() {
    ...
    const getPurchaseOrders = async () => {
      await axios
        .get(base_url + 'dealer/purchase-orders/pending/data?per_page=' + per_page.value)
        .then(response => {
    ...
    const submitApproval = async (purchase_order, status) => {
      ...
      await axios
        .post(`${base_url}dealer/purchase-orders/status/update`, {
    ...
    watch([per_page], ([per_page]) => {
      axios
        .get(base_url + 'dealer/purchase-orders/data?per_page=' + per_page, {
          params: {
    ...
    const exportReport = () => {
      axios
        .get(base_url + 'dealer/purchase-orders/data/export', {
        ...
        .then(response => {
          ...
          fileLink.setAttribute('download', `Purchase_orders_${moment().format('Do_MMM_YYYY')}.xlsx`);
    ...
    const submitBulkAction = action => {
      ...
      axios
        .post(base_url + 'dealer/purchase-orders/bulk/approve', formData)
        .then(() => {
