<template>
    ...
    <table class="table">
      <tbody class="table-border-bottom-0">
        <tr class="text-nowrap" v-for="tax in taxes" :key="tax.id">
          ...
          <td>
            <div v-if="can_edit == 1">
              <i class="tf-icons ti ti-trash ti-xs me-1 text-danger" @click="deleteTax(tax.id)" style="cursor: pointer;"></i>
            </div>
          </td>
        </tr>
      </tbody>
    ...
    <form @submit.prevent="addTax" method="post">
      ...
      <tbody class="table-border-bottom-0">
        <tr class="text-nowrap">
          ...
          <td>
            <div class="">
              <input type="number" step=".01" min="0" class="form-control" v-model="tax_value" placeholder="Tax Value">
            </div>
          </td>
        </tr>
      </tbody>
      ...
      <div class="d-flex px-3">
        <button class="btn btn-primary" type="submit" v-if="can_edit == 1">
          {{ $t('Add')}}
        </button>
      </div>
    </form>
  ...
  export default {
    ...
    setup(props, context) {
      ...
      const deleteTax = id => {
        axios.delete(`settings/taxes/${id}`)
          .then(() => {
            getTaxes()
