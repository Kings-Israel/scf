        ...
        <table class="table">
          ...
          <tbody class="table-border-bottom-0">
            ...
            <tr v-for="cbs_transaction in reports.data" :key="cbs_transaction.id">
              ...
              <td>{{ new Intl.NumberFormat().format(cbs_transaction.amount) }}</td>
              <td>{{ new Intl.NumberFormat().format(cbs_transaction.amount) }}</td>
              <td>-</td>
              <td>-</td>
            </tr>
...
export default {
  ...
  setup(props) {
    ...
    watch([per_page], async ([per_page]) => {
      await axios
        .get(base_url + 'dealer/invoices/od-accounts/' + props.program_vendor_configuration + '/cbs-transactions', {
          params: {
            per_page: per_page,
            payment_account_number: payment_account_number_search.value,
            dealer: dealer_search.value
