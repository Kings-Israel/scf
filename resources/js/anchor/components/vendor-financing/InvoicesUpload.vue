      ...
      >
        ...
        <div class="modal-body">
          <div class="form-group d-none">
            ...
            <select name="product_type" id="upload-product-type" class="form-control" @change="changeProductType">
              <option value="">{{ $t('Select Type') }}</option>
              <option value="Vendor Financing Receivable">{{ $t('Vendor Financing Receivable') }}</option>
              <option value="Dealer Financing" v-if="dealer_financing == '1'">{{ $t('Dealer Financing') }}</option>
            </select>
          </div>
        ...
        <div class="modal-footer">
          ...
          <a href="#" id="download-dealer-invoices" target="_blank" class="btn btn-label-warning d-none">{{
            $t('Download Sample')
          }}</a>
          <a href="invoices/vendors" target="_blank" id="view-vendors" class="btn btn-label-info">{{
            $t('View Vendors')
          }}</a>
          <a href="#" target="_blank" id="view-dealers" class="btn btn-label-info d-none">{{ $t('View Dealers') }}</a>
...
export default {
  ...
  setup(props) {
    ...
    const uploadInvoices = () => {
      ...
      axios
        .post(base_url + 'invoices/import', formData)
        .then(response => {
    ...
    const changeProductType = () => {
      let value = $('#upload-product-type').val();
      ...
      } else if (value == 'Dealer Financing') {
