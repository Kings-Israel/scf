...
<script>
  let items_count = {!! json_encode($purchase_order->purchaseOrderItems->count()) !!}
  let taxes_count = {!! json_encode($purchase_order->purchaseOrderItems->count()) !!}
  let items = $('#items')
  ...
  var min_invoice_day = new Date(purchase_order.delivery_date)
  var invoice_dd = min_invoice_day.getDate();
  var invoice_mm = min_invoice_day.getMonth() + 1; //January is 0!
  var invoice_yyyy = min_invoice_day.getFullYear();

  if (invoice_dd < 10) {
    invoice_dd = '0' + invoice_dd;
  }

  if (mm < 10) {
    invoice_mm = '0' + invoice_mm;
  }

  min_invoice_day = invoice_yyyy + '-' + invoice_mm + '-' + invoice_dd;
  $('#invoice_date').attr("min", min_invoice_day);
          ...
          <div id="account-details-vertical" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                ...
                <input type="hidden" name="anchor" value="{{ $purchase_order->anchor->id }}" />
                ...
                @if ($purchase_order)
                  @foreach ($purchase_order->purchaseOrderItems as $key => $purchase_order_item)
                    ...
                    <div class="col-sm-2">
                      ...
                      <input type="text" name="quantity[{{ $key }}]" value="{{ number_format($purchase_order_item->quantity) }}" class="form-control" placeholder="0" id="quantity-{{ $key }}" oninput="updateTotal(0)" onblur="calculateSubtotal()" aria-label="quantity" required />
                    </div>
                    <div class="col-sm-2">
                    ...
                    <div class="col-sm-2">
                      ...
                      <input type="text" name="price_per_quantity[{{ $key }}]" value="{{ number_format($purchase_order_item->price_per_quantity) }}" class="form-control" id="price-{{ $key }}" placeholder="Ksh" oninput="updateTotal(0)" onblur="calculateSubtotal()" aria-label="price_per_quantity" required />
                    </div>
                    <div class="col-sm-3">
