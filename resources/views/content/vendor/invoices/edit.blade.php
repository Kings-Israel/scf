...
<script>
  ...
  if (invoice) {
    $(document).ready(function () {
      setTimeout(() => {
        console.log(invoice.total_invoice_taxes)
        calculateSubtotal()
  ...
  $('#purchase_order').on('change', function () {
    ...
    if (po_val != '' && po_val != 'Select Purchase Order') {
      $.get(`../purchase-orders/${po_val}/show`, function (data, status) {
        ...
        items_count = 0
        data.data.purchase_order_items.forEach((item, index) => {
          let html = '<div class="col-sm-3" id="item-div-'+items_count+'">'
              html += '<label class="form-label" for="item">Item</label>'
              ...
              html += '<div class="col-sm-4" id="discount-type-'+items_count+'">'
              html += '<label class="form-label" for="Discount Type">Discount Type</label>'
              html += '<select name="discount_type['+items_count+']" id="discount_type-0" class="form-control">'
              html += '<option value="">Select</option>'
              html += '<option value="percentage">Percentage</option>'
              html += '<option value="absolute">Absolute</option>'
              html += '</select>'
  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper vertical wizard-vertical-icons-example mt-2">
      ...
      <div class="bs-stepper-content">
        <form method="POST" action="{{ route('vendor.invoice-update', ['invoice' => $invoice]) }}" enctype="multipart/form-data" id="invoice-form">
          @csrf
          <!-- Account Details -->
          <div id="account-details-vertical" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                ...
                <select class="select2" id="anchor" name="anchor" required>
                  <option label=" "></option>
              ...
              <div class="col-sm-6">
                ...
                <input type="text" id="invoice_number" name="invoice_number" class="form-control" placeholder="Invoice Number" aria-label="invoice_number" value="{{ old('invoice_number', $invoice ? $invoice->invoice_number : '') }}" required />
                ...
                <div class="row">
                  ...
                  <div class="col-sm-12 col-md-6">
                    ...
                    <select class="form-select" id="credit_to_options" name="credit_to" required>
                      @foreach ($vendor_bank_details as $vendor_bank)
                        <option value="{{ $vendor_bank->account_number }}" @if($invoice->credit_to == $vendor_bank->account_number) selected @endif>{{ $vendor_bank->account_number }}</option>
                      @endforeach
                    </select>
