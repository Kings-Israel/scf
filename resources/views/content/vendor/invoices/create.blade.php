...
<script>
  ...
  function updateTaxes(item_index, index) {
    ...
    // let previous_tax = $('#tax-name-'+item_index+'-'+(index - 1)).find(':selected').data('value');
    // if (previous_tax) {
    //   if ($('#tax-name-'+item_index+'-'+index).find(':selected').val() == $('#tax-name-'+item_index+'-'+(index - 1)).find(':selected').val()) {
    //     $('#tax-name-'+item_index+'-'+(index - 1)).val('')
    //     return
    //   }
    // }
  ...
  $('#invoice_number').on('input', function (e) {
    ...
    if ($(this).val().length > 3) {
      $.ajax({url: `invoice-number/${$(this).val()}/check`,
  ...
  let program_vendor_configurations = []
  ...
  $('#anchor').on('change', function (e) {
    ...
    $.get(url, function (data, status) {
      program_vendor_configurations = data.programs
  ...
  $('#program_options').on('change', function () {
    ...
    $.ajax({
      url: `../invoices/program/${$(this).val()}/${anchor}`,
  ...
  $('#purchase_order').on('change', function () {
    ...
    if (po_val != '' && po_val != 'Select Purchase Order') {
      $.get(`../purchase-orders/${po_val}/show`, function (data, status) {
        ...
        $('#anchor').val(data.anchor_id)

        let url = invoice ? `../create/${data.anchor_id}/programs` : `create/${data.anchor_id}/programs`
        $.get(url, function (data, status) {
          program_vendor_configurations = data.programs
          let programOptions = document.getElementById('program_options')
        ...
        data.purchase_order_items.forEach((item, index) => {
          let html = '<div class="col-sm-3" id="item-div-'+items_count+'">'
              html += '<label class="form-label" for="item">Item</label>'
              html += '<input type="text" name="item['+items_count+']" class="form-control" placeholder="Item" autocomplete="off" aria-label="item" value="'+item.item+'" />'
              html += '</div>'
              html += '<div class="col-sm-2" id="quantity-div-'+items_count+'">'
              html += '<label class="form-label" for="quantity">Quantity</label>'
              html += '<input type="text" name="quantity['+items_count+']" value="'+Number(item.quantity).toLocaleString()+'" autocomplete="off" class="form-control" min="1" id="quantity-'+items_count+'" oninput="updateTotal('+items_count+')" onblur="calculateSubtotal()" placeholder="0" aria-label="quantity" required />'
              html += '</div>'
              html += '<div class="col-sm-2" id="unit-div-'+items_count+'">'
              html += '<label class="form-label" for="unit">Unit</label>'
              html += '<input type="text" name="unit['+items_count+']" value="'+item.unit+'" class="form-control" autocomplete="off" placeholder="0" aria-label="unit" />'
              html += '</div>'
              html += '<div class="col-sm-2" id="price-per-quantity-div-'+items_count+'">'
              html += '<label class="form-label" for="price_per_quantity">Price Per Quantity</label>'
              html += '<input type="text" name="price_per_quantity['+items_count+']" autocomplete="off" value="'+Number(item.price_per_quantity).toLocaleString()+'" class="form-control" min="1" id="price-'+items_count+'" placeholder="Ksh" oninput="updateTotal('+items_count+')" onblur="calculateSubtotal()" aria-label="price_per_quantity" required />'
              html += '</div>'
              html += '<div class="col-sm-3" id="total-div-'+items_count+'">'
              html += '<label class="form-label" for="total">Total</label>'
...
<script>
  ...
  Dropzone.options.documentDropzone = {
    url: '{{ route('vendor.invoices.attachment.store') }}',
    maxFilesize: 4, // MB
  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper vertical wizard-vertical-icons-example mt-2">
        ...
        <div class="step" data-target="#account-details-vertical">
          <button type="button" class="step-trigger">
            ...
            <span class="bs-stepper-label">
              ...
              <span class="bs-stepper-subtitle">{{ __('PO No, Anchor, A/C')}}</span>
            </span>
          </button>
        </div>
      ...
      <div class="bs-stepper-content">
        <form method="POST" action="{{ route('vendor.invoice-store') }}" enctype="multipart/form-data" id="invoice-form">
          @csrf
          <!-- Account Details -->
          <div id="account-details-vertical" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                ...
                <select class="form-select form-control" id="anchor" name="anchor" required>
                  <option label="" value="">{{ __('Select Anchor') }}</option>
                  @foreach ($anchors as $anchor)
                    <option value="{{ $anchor->id }}" @if($invoice && $invoice->program->anchor->id == $anchor->id || old('anchor') == $anchor->id) selected @endif>{{ $anchor->name }}</option>
                  @endforeach
                </select>
                <x-input-error :messages="$errors->get('anchor')" />
              </div>
              <div class="col-sm-6">
              ...
              <div class="col-sm-6">
                ...
                <select class="form-control" id="purchase_order" name="purchase_order">
                  <option label="">{{ __('Select Purchase Order')}}</option>
                ...
                @if ($invoice)
                  @foreach ($invoice->invoiceItems as $key => $invoice_item)
                    ...
                    <div class="col-sm-2">
                      ...
                      <input type="text" name="price_per_quantity[{{ $key }}]" value="{{ $invoice_item->price_per_quantity }}" class="form-control" id="price-{{ $key }}" placeholder="Ksh" oninput="updateTotal({{ $key }})" onblur="calculateSubtotal()" aria-label="price_per_quantity" required />
              ...
              <div class="col-12">
                <div class="row">
                  ...
                  <div class="col-sm-12 col-md-6">
                    ...
                    <select class="form-select" id="credit_to_options" name="credit_to" required>
                      @if ($invoice && count($vendor_bank_accounts) > 0)
                        @foreach ($vendor_bank_accounts as $vendor_bank_account)
                          <option value="{{ $vendor_bank_account->account_number }}" @if($invoice->credit_to == $vendor_bank_account->account_number) selected @endif>{{ $vendor_bank_account->account_number }}</option>
                        @endforeach
                      @endif
                      @if (old('credit_to'))
