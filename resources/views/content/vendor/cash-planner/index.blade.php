...
<script>
  ...
  $('#select_anchor').on('change', function (e) {
    e.preventDefault();
    $.get(`../invoices/create/${$(this).val()}/programs`, function (data, status) {
  ...
  $('#program_options').on('change', function () {
    ...
    $('#pi_amount').val(0)
    $.get(`../cash-planner/program/${$(this).val()}/financing/eligible`, function (data, status) {
  ...
  $('.payment_date').on('change', function () {
    ...
    $.get(`../cash-planner/program/${program_id}/${$(this).val()}/financing/calculate`,
  ...
  $('#calculator_program').on('change', function() {
    ...
    if ($('#calculator_pi_amount').val() != '') {
      ...
      $.get(`../cash-planner/program/planner/calculate`,
      {
        program: program_id,
        amount: amount,
        date: date,
...
<div id="vendor-cash-planner-programs">
  <vendor-cash-planner-programs-component></vendor-cash-planner-programs-component>
</div>
<br>
<div class="card">
  <div class="card-header">
    <h6>{{ __('Fund Planner For Vendor Financing') }}</h6>
  </div>
  <div class="card-body">
    <form action="{{ route('vendor.cashplanner.financing_requests.store') }}" method="post">
      @csrf
      <div class="row clearfix">
        <div class="col-sm-6">
          <label class="form-label" for="select_anchor">{{ __('Select Anchor') }}</label>
        ...
        <div class="col-12 d-flex mt-2">
          <button class="btn btn-primary disabled" type="submit" id="submit-invoices"> <span class="align-middle d-sm-inline-block d-none me-sm-1">{{ __('Submit') }}</span></button>
        </div>
      </div>
    </form>
    <form action="{{ route('vendor.cashplanner.financing_requests.export') }}" method="post">
      @csrf
        ...
        <div class="col-sm-3">
          ...
          <select class="form-select select2" id="calculator_program" name="program_id">
            ...
            @foreach ($programs as $program)
              <option value="{{ $program->program->id }}" data-program="{{ $program->program }}">{{ $program->payment_account_number }} ({{ $program->program->anchor->name }})</option>
            @endforeach
          </select>
        </div>
        <div class="col-sm-3">
          <label class="form-label" for="associated_po">{{ __('Payment Instruction Amount') }}</label>
          <input type="number" step=".01" id="calculator_pi_amount" class="form-control" min="1" autocomplete="off" />
        </div>
        <div class="col-sm-3">
...
<br>
<div id="vendor-cash-planner-eligible-invoices">
  <vendor-cash-planner-eligible-invoices-component can_request={{ auth()->user()->hasPermissionTo('Request Vendor Finance') }}></vendor-cash-planner-eligible-invoices-component>
</div>
<br>
<div id="vendor-cash-planner-non-eligible-invoices">
  <vendor-cash-planner-non-eligible-invoices-component></vendor-cash-planner-non-eligible-invoices-component>
</div>
@endsection
