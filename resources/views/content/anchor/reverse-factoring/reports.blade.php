...
<script>
  ...
  $(document).ready(function () {
    ...
    let earningReportsTabsOrders = new ApexCharts(earningReportsTabsOrdersEl, earningReportsTabsOrdersConfig);
    ...
    let earningReportsTabsSales = new ApexCharts(earningReportsTabsSalesEl, earningReportsTabsSalesConfig);
    ...
    let earningReportsTabsIncome = new ApexCharts(earningReportsTabsIncomeEl, earningReportsTabsIncomeConfig);
    ...
    let projectStatus = new ApexCharts(projectStatusEl, projectStatusConfig);
    projectStatus.render();

    $('#timeline-filter').on('change', function() {
      let value = $(this).val();

      assets = 'reports/data?timeline='+value;
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          projectStatus = new ApexCharts(projectStatusEl, statusConfig)
          projectStatus.render();
        },
        error: function (err) {
          console.log(err)
    ...
    $('#invoices-timeline-filter').on('change', function() {
      let value = $(this).val();
      ...
      $.ajax({
        ...
        async: true,
        success: function(data) {
          let reportsTabsOrdersConfig = EarningReportsBarChart(data.all_invoices, data.months);
          let reportsTabsSalesConfig = EarningReportsBarChart(data.pending_invoices, data.months);
          let reportsTabsProfitConfig = EarningReportsBarChart(data.discount_amounts, data.months);
          ...
          earningReportsTabsIncome.destroy();
          earningReportsTabsIncome = new ApexCharts(earningReportsTabsIncomeEl, reportsTabsProfitConfig);
          earningReportsTabsIncome.render()
        },
        error: function (err) {
          console.log(err)
  ...
  <div class="col-xl-4 col-md-4 col-6 mb-4">
    <div class="card h-100">
      <div class="card-header d-flex justify-content-between">
        ...
        <div class="timeline-filter-container p-0">
      ...
      <div class="card-body">
        <div class="d-flex align-items-start">
          ...
          <div class="d-flex justify-content-between w-100 gap-2 align-items-center">
            <div class="me-2">
              <h6 class="mb-0">{{ $default_currency }} {{ number_format($total_program_limit) }}</h6>
            </div>
          </div>
        </div>
        ...
        <div class="d-flex justify-content-between mb-3">
          ...
          <div class="d-flex">
            <p class="mb-0 me-3">{{ $default_currency }}. {{ number_format($available_amount) }}</p>
          </div>
        </div>
        <div class="d-flex justify-content-between mb-3 pb-1">
          <h6 class="mb-0">{{ __('Utilized Limit')}}</h6>
          <div class="d-flex">
            <p class="mb-0 me-3">{{ $default_currency }}. {{ number_format($utilized_amount) }}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="col-12 col-xl-8 mb-4 order-1 order-xl-2">
    <div class="card h-100">
      <div class="card-header d-flex justify-content-between">
        ...
        <div class="timeline-filter-container p-0">
          <label for="timeline-filter" class="form-label">{{ __('Select Timeline')}}</label>
          <select class="form-select" id="invoices-timeline-filter" name="timeline_filter">
            <option value="past_five_years">{{ __('Past 5 Years')}}</option>
            <option value="past_three_years">{{ __('Past 3 Years')}}</option>
            <option value="past_year" selected>{{ __('Past Year')}}</option>
            <option value="past_six_months">{{ __('Past 6 Months')}}</option>
...
<div id="anchor_reports">
  <anchor-reports
    can_view_all_invoices={{ auth()->user()->hasPermissionTo('View All Invoices Report') ? 1 : 0 }}
    can_view_programs={{ auth()->user()->hasPermissionTo('View Financing Usage') ? 1 : 0 }}
    can_view_payments={{ auth()->user()->hasPermissionTo('View PI Listing Report') ? 1 : 0 }}
  ></anchor-reports>
</div>
