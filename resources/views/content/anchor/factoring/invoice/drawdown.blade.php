...
<script>
  ...
  $('#invoice_number').on('input', function (e) {
    ...
    if ($(this).val().length > 3) {
      $.ajax({url: `../create/invoice-number/${$(this).val()}/check/${dealer}`,
  ...
  $('#program_options').on('change', function () {
    let dealer = $('#dealer').val()
    $.ajax({
      url: `../../programs/${$(this).val()}/${dealer}/details`,
  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper vertical wizard-vertical-icons-example mt-2">
      ...
      <div class="bs-stepper-content">
        <form method="POST" action="{{ route('anchor.factoring-invoices-drawdown-store') }}" enctype="multipart/form-data" id="invoice-form">
          @csrf
          <!-- Account Details -->
          <div id="account-details-vertical" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                ...
                <select class="select2" id="dealer" name="dealer" required>
                  <option label=" "></option>
                  @foreach ($dealers as $dealer)
                    <option value="{{ $dealer->id }}" @if($invoice && $invoice->company->id == $dealer->id || old('dealer') == $dealer->id) selected @endif>{{ $dealer->name }}</option>
                  @endforeach
                </select>
                <x-input-error :messages="$errors->get('dealer')" />
              </div>
              <div class="col-sm-6">
              ...
              <div class="col-sm-6">
                ...
                <input class="form-control invoice-date" name="invoice_date" type="date" value="{{ old('invoice_date', $invoice ? Carbon\Carbon::parse($invoice->invoice_date)->format('Y-m-d') : '') }}" id="invoice_date" required />
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-md" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="modalCenterTitle">{{ __('Add Attachment') }}</h5>
                        </div>
