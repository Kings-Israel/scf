...
<script>
    ...
    if ($(this).is(':checked')) {
      ...
      program.fees.forEach(program_fee => {
        let html = '<div class="col-sm-4" id="fee-name-'+fees_count+'">'
            ...
            html += '</div>'
            if (program_fee.type == 'per amount') {
              html += '<div class="col-sm-4" id="fee-per-amount-value-'+fees_count+'">'
              html += '<label class="form-label" for="value">Amount</label>'
              html += '<input type="number" id="fee-per-amount-value-input-'+fees_count+'" step=".00000001" class="form-control" name="fee_per_amount['+fees_count+']" value="'+program_fee.per_amount+'" readonly />'
              html += '</div>'
            } else {
              html += '<div class="col-sm-4 d-none" id="fee-per-amount-value-'+fees_count+'">'
              html += '<label class="form-label" for="value">Amount</label>'
              html += '<input type="number" id="fee-per-amount-value-input-'+fees_count+'" step=".00000001" class="form-control" name="fee_per_amount['+fees_count+']" readonly />'
              html += '</div>'
            }
            html += '<div class="col-sm-4" id="fee-values-'+fees_count+'">'
            html += '<label class="form-label" for="value">Value</label>'
            html += '<input type="number" step=".00000001" id="fee-value-'+fees_count+'" class="form-control" name="fee_values['+fees_count+']" value="'+program_fee.value+'" />'
            html += '</div>'
            ...
            html += '<input type="number" id="vendor-fee-bearing-'+fees_count+'" class="form-control" step=".00000001" value="'+program_fee.vendor_bearing_discount+'" name="fee_vendor_bearing_discount['+fees_count+']" readonly />'
            html += '</div>'
  ...
  $(document.body).on('click', '#add-item', function (e) {
    e.preventDefault()
    let html = '<input hidden value="-1" id="fee-key-'+fees_count+'" name="fee_key['+fees_count+']" />'
        ...
        html += '<div class="col-sm-4" id="fee-anchor-bearing-'+fees_count+'">'
        html += '<label class="form-label" for="anchor-fee-bearing">Anchor Bearing (%)</label>'
        html += '<input type="number" id="anchor-fee-bearing-'+fees_count+'" class="form-control" min="0" step=".00000001" max="100" name="fee_anchor_bearing_discount['+fees_count+']" oninput="updateFeeBearing('+fees_count+')" />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-vendor-bearing-'+fees_count+'">'
        html += '<label class="form-label" for="vendor-fee-bearing">Vendor Bearing (%)</label>'
  ...
  function removeFee(index) {
    $('div').remove('#fee-name-'+index+', #fee-type-'+index+', #fee-values-'+index+', #fee-anchor-bearing-'+index+', #fee-vendor-bearing-'+index+', #fee-per-amount-value-'+index+', #fee-taxes-'+index+', #fee-delete-'+index)
    fees_count -= 1;
  }
...
@section('content')
<h4 class="mb-2">
  <span class="fw-light">{{ __('Edit Mapping of Vendor')}}, {{ $company->name }}, {{ __('to Program')}} - <a href="{{ route('programs.show', ['bank' => $bank, 'program' => $program]) }}" class="text-primary text-decoration-underline">{{ $program->name }}</a></span>
</h4>
<div class="">
  @if($errors->any())
  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper wizard-vertical vertical mt-2" id="vendor-details-wizard">
      ...
      <div class="bs-stepper-content">
        <form id="vendor-details-form" method="POST" action="{{ route('programs.vendors.map.update', ['bank' => $bank, 'program' => $program, 'company' => $company]) }}">
          @csrf
          <!-- Company Details -->
          <div id="vendor-details" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                <label class="form-label" for="buyer">{{ __('Vendor')}} <span class="text-danger">*</span></label>
              ...
              <div class="col-sm-6">
                ...
                <input type="text" id="payment-od-account" class="form-control" name="payment_account_number" @if($program->programCode->name == 'Factoring With Recourse') readonly @endif value="{{ $mapping->configuration->payment_account_number }}" />
          ...
          <div id="discount-details" class="content">
            <div class="row g-3">
              ...
              <div class="col-sm-6">
                <label class="form-label" for="benchmark-title">{{ __('Benchmark Title(Maturity)')}}</label>
              ...
              <div class="col-sm-6">
                <div class="d-flex justify-content-between">
                  <label class="form-label" for="current-brnchmark-rate">{{ __('Current Base Rate')}} <span class="text-danger">*</span></label>
                  <span class="text-primary">{{ __('Set As Per Current Master')}}</span>
                </div>
                <input type="number" readonly id="current-brnchmark-rate" class="form-control" min="0" max="100" step=".00000001" readonly value="{{ $mapping->discounts->benchmark_rate }}" name="benchmark_rate" />
              ...
              <div class="col-12 row" id="program-fees">
                @foreach ($mapping->fees as $key => $fee)
                  @if ($loop->first)
                    ...
                    <div class="col-sm-4">
                      <label class="form-label" for="ancor-discount-bearing">{{ __('Anchor Fee Bearing')}} (%)</label>
                    ...
                    <div class="col-sm-4">
                      <label class="form-label" for="vendor-discount-bearing">{{ __('Vendor Fee Bearing')}} (%)</label>
                  ...
                  @else
                    ...
                    <div class="col-sm-4" id="fee-anchor-bearing-{{ $key }}">
                      ...
                      <input type="number" id="anchor-discount-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".00000001" name="fee_anchor_bearing_discount[{{ $key }}]" value="{{ $fee->anchor_bearing_discount }}" oninput="updateFeeBearing({{ $key }})" />
                    </div>
                    <div class="col-sm-4" id="fee-vendor-bearing-{{ $key }}">
                      <label class="form-label" for="vendor-discount-bearing">{{ __('Vendor Bearing')}} (%)</label>
                      <input type="text" id="vendor-discount-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".00000001" readonly name="fee_vendor_bearing_discount[{{ $key }}]" value="{{ $fee->vendor_bearing_discount }}" />
                    </div>
                    <div class="col-sm-4" id="fee-charge-types-{{ $key }}">
                      <label class="form-label" for="value">{{ __('Charge')}}</label>
                      <select class="form-select" id="fee-charge-type-{{ $key }}" name="charge_types[{{ $key }}]">
                ...
                <div class="col-sm-6">
                  ...
                  <select class="form-select" id="bank-name-{{ $key }}" name="bank_names[{{ $key }}]" onchange="getSwiftCode({{ $key }})">
                    @foreach ($banks as $vendor_bank)
                      <option value="{{ $vendor_bank->name }}" data-swiftcode="{{ $vendor_bank->swift_code }}" @if($bank_details->bank_name == $vendor_bank->name) selected @endif>{{ $vendor_bank->name }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="col-sm-6">
