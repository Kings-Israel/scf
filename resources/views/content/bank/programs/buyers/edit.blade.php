...
@section('title', 'Edit Buyer Mapping')
...
<script>
  ...
  $('#flexSwitchCheckChecked').click(function () {
    if ($(this).is(':checked')) {
      ...
      fees.text('')
      program.fees.forEach(program_fee => {
        let html = '<div class="col-sm-4" id="fee-name-'+fees_count+'">'
            html += '<label class="form-label" for="fee-name">Fee Name</label>'
            html += '<input type="text" id="fee-name" class="form-control" name="fee_names['+fees_count+']" value="'+program_fee.fee_name+'" />'
            html += '</div>'
            ...
            if (program_fee.type == 'per amount') {
              html += '<option value="per amuont" selected>Per Amount</option>'
            } else {
              html += '<option value="per amuont">Per Amount</option>'
            }
            ...
            if (program_fee.type == 'per amount') {
              ...
              html += '<input type="number" id="fee-per-amount-value-input-'+index+'" step=".00000001" class="form-control" name="fee_per_amount['+index+']" value="'+program_fee.per_amount+'" readonly />'
              html += '</div>'
            } else {
              html += '<div class="col-sm-4 d-none" id="fee-per-amount-value-'+index+'">'
              html += '<label class="form-label" for="value">Amount</label>'
              html += '<input type="number" id="fee-per-amount-value-input-'+index+'" step=".00000001" class="form-control" name="fee_per_amount['+index+']" readonly />'
              html += '</div>'
            }
            html += '</div>'
            html += '<div class="col-sm-4" id="fee-values-'+fees_count+'">'
            html += '<label class="form-label" for="value">Value</label>'
            html += '<input type="number" id="fee-value-'+fees_count+'" class="form-control" name="fee_values['+fees_count+']" value="'+program_fee.value+'" />'
            html += '</div>'
            html += '<div class="col-sm-4" id="fee-anchor-bearing-'+fees_count+'">'
            html += '<label class="form-label" for="anchor-fee-bearing">Buyer Bearing (%)</label>'
            html += '<input type="number" id="anchor-fee-bearing-'+fees_count+'" class="form-control" min="0" max="100" step=".00000001" value="'+program_fee.anchor_bearing_discount+'" name="fee_anchor_bearing_discount['+fees_count+']" oninput="updateFeeBearing('+fees_count+')" />'
            html += '</div>'
            ...
            html += '<input type="number" id="vendor-fee-bearing-'+fees_count+'" class="form-control" value="'+program_fee.vendor_bearing_discount+'" step=".00000001" name="fee_vendor_bearing_discount['+fees_count+']" readonly />'
            html += '</div>'
  ...
  $(document.body).on('click', '#add-item', function (e) {
    ...
    let html = '<div class="col-sm-4" id="fee-name-'+fees_count+'">'
        ...
        html += '<label class="form-label" for="value">Amount</label>'
        html += '<input type="number" step=".00000001" id="fee-per-amount-value-input-'+fees_count+'" class="form-control" name="fee_per_amount['+fees_count+']" />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-values-'+fees_count+'">'
        html += '<label class="form-label" for="value">Value</label>'
        ...
        html += '<div class="col-sm-4" id="fee-anchor-bearing-'+fees_count+'">'
        html += '<label class="form-label" for="anchor-fee-bearing">Buyer Bearing (%)</label>'
        html += '<input type="number" step=".00000001" id="anchor-fee-bearing-'+fees_count+'" class="form-control" min="0" max="100" name="fee_anchor_bearing_discount['+fees_count+']" oninput="updateFeeBearing('+fees_count+')" />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-vendor-bearing-'+fees_count+'">'
        html += '<label class="form-label" for="vendor-fee-bearing">Vendor Bearing (%)</label>'
        html += '<input type="number" step=".00000001" id="vendor-fee-bearing-'+fees_count+'" class="form-control" name="fee_vendor_bearing_discount['+fees_count+']" readonly />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-charge-types-'+fees_count+'">'
        html += '<label class="form-label" for="value">Charge</label>'
        html += '<select class="form-select" id="fee-charge-types-'+fees_count+'" name="charge_types['+fees_count+']">'
  ...
  function removeFee(index) {
    $('div').remove('#fee-name-'+index+', #fee-type-'+index+', #fee-values-'+index+', #fee-anchor-bearing-'+index+', #fee-vendor-bearing-'+index+', #fee-per-amount-value-'+index+', #fee-charge-types-'+index+', #fee-account-numbers-'+index+', #fee-taxes-'+index+', #fee-delete-'+index)
    fees_count -= 1;
  }
...
<h4 class="fw-bold mb-2 d-flex justify-content-between">
  <span class="fw-light">{{ __('Edit Mapping of Buyer')}}, {{ $company->name }}, {{ __('to Program')}} - <a href="{{ route('programs.show', ['bank' => $bank, 'program' => $program]) }}" class="text-primary text-decoration-underline">{{ $program->name }}</a></span>
  <div class="d-flex">
  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper wizard-vertical vertical mt-2" id="vendor-details-wizard">
        ...
        <div class="step" data-target="#vendor-details">
          <button type="button" class="step-trigger">
            ...
            <span class="bs-stepper-label">
              <span class="bs-stepper-title">{{ __('Buyer Details')}}</span>
            </span>
          </button>
        </div>
        <div class="line"></div>
      ...
      <div class="bs-stepper-content">
        <form id="vendor-details-form" method="POST" action="{{ route('programs.buyers.map.update', ['bank' => $bank, 'program' => $program, 'company' => $company]) }}">
          @csrf
          <!-- Company Details -->
          <div id="vendor-details" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                <label class="form-label" for="buyer">{{ __('Buyer')}} <span class="text-danger">*</span></label>
              ...
              <div class="col-sm-6">
                ...
                <input type="text" id="payment-od-account" class="form-control" name="payment_account_number" value="{{ $mapping->configuration->payment_account_number }}" required />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="eligibility" class="form-control" name="eligibility" min="0.01" step=".01" max="100" value="{{ $mapping->configuration->eligibility }}" required />
              ...
              <div class="col-sm-6">
                ...
                <input type="text" id="sanctioned_limit" class="form-control" name="sanctioned_limit" max="{{ $sanctioned_limit }}" value="{{ $mapping->configuration->sanctioned_limit }}" required />
              ...
              <div class="col-sm-6">
                ...
                <input type="text" id="drawing_power" class="form-control" name="drawing_power" max="{{ $sanctioned_limit }}" value="{{ $mapping->configuration->drawing_power ? $mapping->configuration->drawing_power : $mapping->configuration->sanctioned_limit }}" />
              ...
              <div class="col-sm-6">
                ...
                <input class="form-control" type="date" id="limit_approval_date" name="limit_approved_date" value="{{ Carbon\Carbon::parse($mapping->configuration->limit_approved_date)->format('Y-m-d') }}" requried />
              ...
              <div class="col-sm-6">
                ...
                <input class="form-control" type="date" id="limit_expiry_date" name="limit_expiry_date" value="{{ Carbon\Carbon::parse($mapping->configuration->limit_expiry_date)->format('Y-m-d') }}" required />
                <x-input-error :messages="$errors->get('limit_expiry_date')" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="limit_review_date">{{ __('Limit Review Date')}} <span class="text-danger">*</span></label>
              ...
              <div class="col-sm-6">
                ...
                <x-input-error :messages="$errors->get('auto_approve_finance')" />
              </div>
              <div class="col-sm-6">
                ...
                <x-input-error :messages="$errors->get('scheme_code')" />
              </div>
              <div class="col-sm-6">
              ...
              <div class="col-sm-6">
                <label class="form-label" for="gst-no">KRA PIN <span class="text-danger">*</span></label>
              ...
              <div class="col-sm-6">
                ...
                <x-input-error :messages="$errors->get('classification')" />
              </div>
              <div class="col-sm-6">
            ...
            <div class="row g-3">
              ...
              <div class="col-sm-6">
                ...
                <select class="form-select" id="benchmark-title" name="benchmark_title" onchange="changeDiscountRates()">
                  ...
                  @foreach ($benchmark_rates as $key => $benchmark_rate)
                    <option value="{{ $key }}" @if($mapping->discounts->benchmark_title == $key) selected @endif data-rate="{{ $benchmark_rate }}">{{ $key }}</option>
                  @endforeach
                </select>
              ...
              <div class="col-sm-6">
                <div class="d-flex justify-content-between">
                  <label class="form-label" for="current-brnchmark-rate">{{ __('Current Base Rate')}}</label>
                  <span class="text-primary">{{ __('Set As Per Current Master')}}</span>
                </div>
                <input type="number" readonly id="current-brnchmark-rate" class="form-control" min="0" max="100" step=".01" readonly value="{{ $mapping->discounts->benchmark_rate }}" name="benchmark_rate" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="business-strategy-spread" class="form-control" name="business_strategy_spread" min="0" max="100" step=".01" readonly value="{{ $mapping->discounts->business_strategy_spread }}" oninput="changeDiscountRates()" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="credit-spread" class="form-control" name="credit_spread" min="0" max="100" step=".01" readonly value="{{ $mapping->discounts->credit_spread }}" oninput="changeDiscountRates()" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="total-spread" class="form-control" name="total_spread" min="0" max="100" step=".01" readonly value="{{ $mapping->discounts->total_spread }}" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="total-roi" class="form-control" name="total_roi" min="0" max="100" step=".01" readonly value="{{ $mapping->discounts->total_roi }}" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="anchor-discount-bearing" class="form-control" min="0" max="100" step=".01" name="anchor_discount_bearing" readonly value="{{ $mapping->discounts->anchor_discount_bearing }}" oninput="changeDiscountRates()" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="vendor-discount-bearing" class="form-control" min="0" max="100" step=".01" name="vendor_discount_bearing" readonly value="{{ $mapping->discounts->vendor_discount_bearing }}" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="penal-discount-on-principle" class="form-control" min="0" max="100" step=".01" name="penal_discount_on_principle" readonly value="{{ $mapping->discounts->penal_discount_on_principle }}" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="grace-period" class="form-control" name="grace_period" readonly value="{{ $mapping->discounts->grace_period }}" />
              ...
              <div class="col-sm-6">
                ...
                <input class="form-control" type="number" id="grace-period-discount" name="grace_period_discount" readonly value="{{ $mapping->discounts->grace_period_discount }}" />
              ...
              <div class="col-sm-6">
                ...
                <select class="form-select" id="maturity-handling-on-holidays" name="maturity_handling_on_holidays">
                  <option value="No Effect" @if($mapping->discounts->maturity_handling_on_holidays == 'No Effect') selected @endif>{{ __('No Effect')}}</option>
                  <option value="Prepone to previous working day" @if($mapping->discounts->maturity_handling_on_holidays == 'Prepone to previous working day') selected @endif>{{ __('Prepone to previous working day')}}</option>
                  <option value="Postpone to next working day" @if($mapping->discounts->maturity_handling_on_holidays == 'Postpone to next working day') selected @endif>{{ __('Postpone to next working day')}}</option>
                </select>
                ...
                @foreach ($mapping->fees as $key => $fee)
                  @if ($loop->first)
                    ...
                    <div class="col-sm-4 @if ($fee->type != 'per amount') d-none @endif" id="fee-per-amount-value-{{ $key }}">
                      ...
                      <input type="number" step=".00000001" class="form-control" name="fee_per_amount[{{ $key }}]" value="{{ $fee->per_amount }}" />
                    </div>
                    <div class="col-sm-4">
                      <label class="form-label" for="value">{{ __('Value')}}</label>
                      <input type="text" id="fee-value-{{ $key }}" class="form-control" name="fee_values[{{ $key }}]" value="{{ $fee->value }}" />
                    </div>
                    <div class="col-sm-4">
                      <label class="form-label" for="ancor-discount-bearing">{{ __('Anchor Discount Bearing')}} (%)</label>
                      <input type="number" id="anchor-fee-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".00000001" name="fee_anchor_bearing_discount[{{ $key }}]" value="{{ $fee->anchor_bearing_discount }}" oninput="updateFeeBearing({{ $key }})" />
                    </div>
                    <div class="col-sm-4">
                      <label class="form-label" for="vendor-discount-bearing">{{ __('Vendor Discount Bearing')}} (%)</label>
                      <input type="text" id="vendor-fee-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".00000001" readonly name="fee_vendor_bearing_discount[{{ $key }}]" value="{{ $fee->vendor_bearing_discount }}" />
                    </div>
                  ...
                  @else
                    ...
                    <div class="col-sm-4" id="fee-anchor-bearing-{{ $key }}">
                      <label class="form-label" for="ancor-discount-bearing">{{ __('Buyer Bearing')}} (%)</label>
                      <input type="number" id="anchor-fee-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".00000001" name="fee_anchor_bearing_discount[{{ $key }}]" value="{{ $fee->anchor_bearing_discount }}" oninput="updateFeeBearing(0)" />
                    </div>
                    <div class="col-sm-4" id="fee-vendor-bearing-{{ $key }}">
                      <label class="form-label" for="vendor-discount-bearing">{{ __('Vendor Bearing')}} (%)</label>
                      <input type="number" id="vendor-fee-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".00000001" readonly name="fee_vendor_bearing_discount[{{ $key }}]" value="{{ $fee->vendor_bearing_discount }}" />
                    </div>
                    <div class="col-sm-4" id="fee-charge-types-{{ $key }}">
                      <label class="form-label" for="value">{{ __('Charge')}}</label>
                      <select class="form-select" id="fee-charge-type-{{ $key }}" name="charge_types[{{ $key }}]">
                    ...
                    <div class="col-sm-12 mb-2 mt-2" id="fee-delete-{{ $key }}">
                      <i class="ti ti-trash ti-sm text-danger" style="cursor: pointer;" onclick="removeFee({{ $key }})"></i>
                    </div>
                  @endif
                @endforeach
              </div>
          ...
          <div id="comm-details" class="content">
            <div class="row g-3">
              <div class="row g-3">
                ...
                <div class="col-sm-6">
                  <label class="form-label" for="role">User's Role</label>
              ...
              @foreach ($mapping->bank_details as $key => $bank_details)
                @if ($loop->first)
                  ...
                  <div class="col-sm-6">
                    ...
                    <select class="form-select" id="bank-name-{{ $key }}" name="bank_names[{{ $key }}]" onchange="getSwiftCode({{ $key }})">
                      @foreach ($banks as $buyer_bank)
                        <option value="{{ $buyer_bank->name }}" @if($bank_details->bank_name == $buyer_bank->name) selected @endif data-swiftcode={{ $buyer_bank->swift_code }}>{{ $buyer_bank->name }}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="col-sm-6">
                  ...
                  <div class="col-sm-6">
                    ...
                    <input type="text" id="swift-code" class="form-control" name="swift_codes[{{ $key }}]" value="{{ $bank_details->swift_code }}" />
                  </div>
                  <div class="col-sm-6">
                ...
                @else
                  ...
                  <div class="col-sm-12" id="delete-item-div-{{ $key }}'">
                    <i class="ti ti-trash ti-sm text-danger mt-4" title="delete" style="cursor: pointer" onclick="deleteItem({{ $key }})"></i>
                  </div>
                  <hr>
                @endif
              @endforeach
