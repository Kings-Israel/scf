...
@section('title', 'Map Buyer To Program')
...
<script>
    ...
    if ($(this).is(':checked')) {
      ...
      program.fees.forEach((program_fee, index) => {
        let html = '<div class="col-sm-4" id="fee-name-'+index+'">'
            ...
            html += '<div class="col-sm-4 d-none" id="fee-per-amount-value-'+index+'">'
            html += '<label class="form-label" for="value">Amount</label>'
            html += '<input type="number" step=".00000001" class="form-control" name="fee_per_amount['+index+']" readonly />'
            html += '</div>'
            html += '<div class="col-sm-4" id="fee-values-'+index+'">'
            html += '<label class="form-label" for="value">Value</label>'
            html += '<input type="number" step=".00000001" id="fee-value-'+index+'" class="form-control" name="fee_values['+index+']" value="'+program_fee.value+'" readonly />'
            html += '</div>'
            ...
            html += '<input type="number" id="anchor-fee-bearing-'+index+'" class="form-control" min="0" max="100" step=".00000001" value="'+program_fee.anchor_bearing_discount+'" name="fee_anchor_bearing_discount['+index+']" oninput="updateFeeBearing('+index+')" readonly />'
            html += '</div>'
            ...
            html += '<input type="number" id="vendor-fee-bearing-'+index+'" class="form-control" step=".00000001" value="'+program_fee.vendor_bearing_discount+'" name="fee_vendor_bearing_discount['+index+']" readonly />'
            html += '</div>'
  ...
  $(document.body).on('click', '#add-item', function (e) {
    ...
    let html = '<div class="col-sm-4" id="fee-name-'+fees_count+'">'
        ...
        html += '<input type="number" id="anchor-fee-bearing-'+fees_count+'" class="form-control" min="0" max="100" step=".00000001" name="fee_anchor_bearing_discount['+fees_count+']" oninput="updateFeeBearing('+fees_count+')" />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-vendor-bearing-'+fees_count+'">'
        html += '<label class="form-label" for="vendor-fee-bearing">Vendor Bearing (%)</label>'
        html += '<input type="number" id="vendor-fee-bearing-'+fees_count+'" class="form-control" step=".00000001" name="fee_vendor_bearing_discount['+fees_count+']" readonly />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-charge-types-'+fees_count+'">'
        html += '<label class="form-label" for="value">Charge</label>'
        html += '<select class="form-select" id="fee-charge-types-'+fees_count+'" name="charge_types['+fees_count+']">'
  ...
  $('#add-bank-details').on('click', function () {
    let html = '<div class="col-sm-6" id="name-as-per-bank-section-'+bank_accounts+'">'
        ...
        html += '<select class="form-select" id="bank-name" name="bank_names['+bank_accounts+']" onchange="getSwiftCode('+bank_accounts+')">'
        ...
        if (data.company.bank_details.length > 0) {
          ...
          data.company.bank_details.forEach((bank_details, index) => {
            let html = '<div class="col-sm-6" id="name-as-per-bank-section-'+index+'">'
                ...
                html += '<label class="form-label" for="bank-name">Bank Name</label>'
                html += '<select class="form-select" id="bank-name-'+index+'" name="bank_names['+index+']">'
                @foreach ($banks as $buyer_bank)
                  if (bank_details.bank_name == {!! json_encode($buyer_bank->name) !!}) {
                    html += '<option value="'+{!! json_encode($buyer_bank->name) !!}+'" data-swiftcode="'+{!! json_encode($buyer_bank->swift_code) !!}+'" selected>'+{!! json_encode($buyer_bank->name) !!}+'</option>'
                  } else {
  ...
  let old_buyer_id = {!! json_encode(old('buyer_id')) !!}

  if (old_buyer_id) {
    $('#bank-accounts').text('');
    $.get({
      "url": '/'+bank.url+'/programs/companies/'+old_buyer_id+'/details',
      "dataType": "json",
      "success": function(data) {
        $('#gst-no').val(data.company.kra_pin)
        if (data.company.bank_details.length > 0) {
...
<div class="d-flex">
  <h4 class="">
    <span class="fw-light">{{ __('Map Buyer to Program')}} - <span class="text-primary text-decoration-underline">{{ $program->name }}</span></span>
  </h4>
  @if($errors->any())
  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper wizard-vertical vertical mt-2" id="vendor-details-wizard">
      ...
      <div class="bs-stepper-content">
        <form id="vendor-details-form" method="POST" action="{{ route('programs.buyers.map.store', ['bank' => $bank, 'program' => $program]) }}">
          @csrf
          <!-- Company Details -->
          <div id="vendor-details" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                ...
                <select class="form-select select2" id="buyer" name="buyer_id">
                  <option value="">Select</option>
                  @foreach ($companies as $company)
                    <option value="{{ $company->id }}" @if(old('buyer_id') == $company->id) selected @endif>{{ $company->name }}</option>
                  @endforeach
                </select>
                <x-input-error :messages="$errors->get('buyer_id')" />
              </div>
              <div class="col-sm-6">
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="eligibility" class="form-control" name="eligibility" min="0.01" step=".01" max="100" value="{{ old('eligibility', $program->eligibility) }}" />
              ...
              <div class="col-sm-6">
                ...
                <x-input-error :messages="$errors->get('limit_approval_date')" />
              </div>
              <div class="col-sm-6">
              ...
              <div class="col-sm-6">
                ...
                <input type="text" id="gst-no" class="form-control" name="gst_number" value="{{ old('gst_number') }}" />
          ...
          <div id="discount-details" class="content">
            <div class="row g-3">
              ...
              <div class="col-sm-6">
                ...
                <input type="number" readonly id="current-benchmark-rate" class="form-control" min="0" max="100" step=".01" readonly value="{{ $program->discountDetails?->first()?->benchmark_rate ?? 0 }}" name="benchmark_rate" />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="business-strategy-spread" class="form-control" name="business_strategy_spread" min="0" max="100" step=".01" readonly value="{{ $program->discountDetails?->first()?->business_strategy_spread ?? 0 }}" oninput="changeDiscountRates()" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="credit-spread">{{ __('Credit Spread')}} (%) <span class="text-danger">*</span></label>
                <input type="number" id="credit-spread" class="form-control" name="credit_spread" min="0" max="100" step=".01" readonly value="{{ $program->discountDetails?->first()?->credit_spread ?? 0 }}" oninput="changeDiscountRates()" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="total-spread">{{ __('Total Spread')}} (%) <span class="text-danger">*</span></label>
                <input type="number" id="total-spread" class="form-control" name="total_spread" min="0" max="100" step=".01" readonly value="{{ $program->discountDetails?->first()?->total_spread ?? 0 }}" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="total-roi">{{ __('Total ROI') }} (%) <span class="text-danger">*</span></label>
                <input type="number" id="total-roi" class="form-control" name="total_roi" min="0" max="100" step=".01" readonly value="{{ $program->discountDetails?->first()?->total_roi ?? 0 }}" />
              </div>
              <div class="col-sm-6">
              ...
              <div class="col-sm-6">
                <label class="form-label" for="maturity-handling-on-holidays">{{ __('Maturity Handling on Holidays')}} <span class="text-danger">*</span></label>
                <select class="form-select" id="maturity-handling-on-holidays" name="maturity_handling_on_holidays" required>
              ...
              <div class="col-12 row" id="program-fees">
                @foreach ($program->fees as $key => $fee)
                  <div class="col-sm-4">
                    ...
                    <input type="text" id="fee-name" class="form-control" name="fee_names[{{ $key }}]" value="{{ $fee->fee_name }}" readonly />
                  </div>
                  <div class="col-sm-4">
                    <label class="form-label" for="fee-type-{{ $key }}">{{ __('Type')}}</label>
                    <select class="form-select" id="fee-type-{{ $key }}" name="fee_types[{{ $key }}]" onchange="changeFeeType({{ $key }})" readonly>
                  ...
                  <div class="col-sm-4 @if ($fee->type != 'per amount') d-none @endif" id="fee-per-amount-value-{{ $key }}">
                    ...
                    <input type="number" class="form-control" name="fee_per_amount[{{ $key }}]" value="{{ $fee->per_amount }}" readonly />
                  </div>
                  <div class="col-sm-4">
                    <label class="form-label" for="value">{{ __('Value')}}</label>
                    <input type="number" step=".01" id="fee-value-{{ $key }}" class="form-control" name="fee_values[{{ $key }}]" value="{{ $fee->value }}" readonly />
                  </div>
                  <div class="col-sm-4">
                    <label class="form-label" for="ancor-discount-bearing">{{ __('Buyer Bearing')}} (%)</label>
                    <input type="number" id="anchor-fee-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="fee_anchor_bearing_discount[{{ $key }}]" value="{{ $fee->anchor_bearing_discount }}" oninput="updateFeeBearing({{ $key }})" readonly />
                  </div>
                  <div class="col-sm-4">
                    <label class="form-label" for="vendor-discount-bearing">{{ __('Vendor Bearing')}} (%)</label>
                    <input type="number" id="vendor-fee-bearing-{{ $key }}" class="form-control" min="0" max="100" step=".01" readonly name="fee_vendor_bearing_discount[{{ $key }}]" value="{{ $fee->vendor_bearing_discount }}" />
                  </div>
                  ...
                  <div class="col-sm-4">
                    ...
                    <select class="form-select" id="taxes" name="taxes[{{ $key }}]" readonly>
                      <option value="">{{ __('Select')}}</option>
                      @foreach ($taxes as $key => $tax)
            ...
            <div class="row g-3" id="bank-accounts">
              ...
              <div class="col-sm-6">
                ...
                <select class="form-select" id="bank-name" name="bank_names[]">
                  @foreach ($banks as $buyer_bank)
                    <option value="{{ $buyer_bank->name }}">{{ $buyer_bank->name }}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-sm-6">
