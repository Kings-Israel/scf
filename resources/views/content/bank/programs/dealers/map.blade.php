...
@section('title', 'Map Dealer To Program')
...
<script>
  ...
  let sanctioned_limit = {!! json_encode($max_limit) !!}
  ...
  let discount_count = discount_details_count
  ...
  $('#flexSwitchCheckChecked').click(function () {
    if ($(this).is(':checked')) {
      ...
      $('#current-dealer-benchmark-rate').val(program.discount_details[0].benchmark_rate)
      discounts.text('')
      ...
      program.fees.forEach((program_fee, index) => {
        ...
        html += '<input type="text" id="fee-name" class="form-control" name="fee_names['+index+']" readonly />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-type-'+index+'">'
        html += '<label class="form-label" for="fee-type-'+index+'">Type</label>'
        html += '<select class="form-select" id="fee-type-'+index+'" name="fee_types['+index+']" onchange="changeFeeType('+index+')" readonly>'
        ...
        html += '<input type="number" step=".00000001" id="fee-per-amoount-value-input-'+index+'" class="form-control" name="fee_per_amount['+index+']" readonly />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-values-'+index+'">'
        html += '<label class="form-label" for="value">Value</label>'
        html += '<input type="number" step=".00000001" id="dealer-fee-value-'+index+'" class="form-control" name="fee_values['+index+']" oninput="updateDealerBearingFee('+index+')" readonly />'
        html += '</div>'
        ...
        html += '<select class="form-select" id="taxes" name="taxes['+index+']" readonly>'
      ...
      program.fees.forEach((program_fee, index) => {
        ...
        $('#dealer-fee-value-'+index).removeAttr('readonly', 'readonly');
  ...
  $(document.body).on('click', '#add-item', function (e) {
    ...
    let html = '<div class="col-sm-4" id="fee-name-'+fees_count+'">'
        ...
        html += '<label class="form-label" for="value">Value</label>'
        html += '<input type="number" step=".00000001" id="dealer-fee-value-'+fees_count+'" class="form-control" name="fee_values['+fees_count+']" oninput="updateDealerBearingFee('+fees_count+')" />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-vendor-bearing-'+fees_count+'">'
        html += '<label class="form-label" for="vendor-fee-bearing">Dealer Bearing</label>'
        html += '<input type="number" step=".00000001" id="dealer-fee-bearing-'+fees_count+'" value="100" class="form-control" name="fee_dealer_bearing_discount['+fees_count+']" />'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-charge-types-'+fees_count+'">'
        html += '<label class="form-label" for="value">Charge</label>'
        html += '<select class="form-select" id="fee-charge-types-'+fees_count+'" name="dealer_charge_types['+fees_count+']">'
        html += '<option value="fixed">Fixed</option>'
        html += '<option value="daily" title="Daily">Per Day</option>'
        html += '</select>'
        html += '</div>'
        html += '<div class="col-sm-4" id="fee-account-numbers-'+fees_count+'">'
        html += '<label class="form-label" for="value">Credit To</label>'
        html += '<select class="form-select" id="fee-account-numbers-'+fees_count+'" name="dealer_fee_account_numbers['+fees_count+']">'
        html += '<option value="">Select Account</option>'
        @foreach ($bank_payment_accounts as $key => $bank_payment_account)
          html += '<option value="'+{!! json_encode($bank_payment_account->account_number) !!}+'">'+{!! json_encode($bank_payment_account->account_number) !!}+' ('+ {!! json_encode($bank_payment_account->account_name) !!} +')</option>'
        @endforeach
  ...
  function changeDiscountRates() {
    ...
    for (let index = 0; index <= discount_details_count; index++) {
  ...
  $('#add-bank-details').on('click', function () {
    let html = '<div class="col-sm-6" id="name-as-per-bank-section-'+bank_accounts+'">'
        ...
        html += '<select class="form-select" id="bank-name-0" name="bank_names['+bank_accounts+']" onchange="getSwiftCode(0)">'
        ...
        html += '<input type="text" id="swift-code-0" class="form-control" name="swift_codes['+bank_accounts+']" />'
        html += '</div>'
        ...
        if (data.company.bank_details.length > 0) {
          ...
          data.company.bank_details.forEach((bank_details, index) => {
            let html = '<div class="col-sm-6" id="name-as-per-bank-section-'+index+'">'
                ...
                html += '<label class="form-label" for="bank-name">Bank Name</label>'
                html += '<select class="form-select" id="bank-name" name="bank_names['+index+']">'
                @foreach ($banks as $dealer_bank)
                  if (bank_details.bank_name == {!! json_encode($dealer_bank->name) !!}) {
                    html += '<option value="'+{!! json_encode($dealer_bank->name) !!}+'" data-swiftcode="'+{!! json_encode($dealer_bank->switft_code) !!}+'" selected>'+{!! json_encode($dealer_bank->name) !!}+'</option>'
                  } else {
                    html += '<option value="'+{!! json_encode($dealer_bank->name) !!}+'" data-swiftcode="'+{!! json_encode($dealer_bank->switft_code) !!}+'">'+{!! json_encode($dealer_bank->name) !!}+'</option>'
                  }
                @endforeach
                html += '</select>'
                html += '</div>'
                ...
                html += '<label class="form-label" for="swift-code">SWIFT Code</label>'
                html += '<input type="text" id="swift-code" class="form-control" name="swift_codes['+index+']" value="'+bank_details.swift_code+'" />'
                html += '</div>'
  ...
  let old_dealer_id = {!! json_encode(old('dealer_id')) !!}

  if (old_dealer_id) {
    $('#bank-accounts').text('');
    $.get({
      "url": '/'+bank.url+'/programs/companies/'+old_dealer_id+'/details',
      "dataType": "json",
      "success": function(data) {
        $('#gst-no').val(data.company.kra_pin)
        if (data.company.bank_details.length > 0) {
          ...
          data.company.bank_details.forEach((bank_details, index) => {
            let html = '<div class="col-sm-6" id="name-as-per-bank-section-'+index+'">'
                ...
                @foreach ($banks as $dealer_bank)
                  if (bank_details.bank_name == {!! json_encode($dealer_bank->name) !!}) {
                    html += '<option value="'+{!! json_encode($dealer_bank->name) !!}+'" selected>'+{!! json_encode($dealer_bank->name) !!}+'</option>'
                  } else {
                    html += '<option value="'+{!! json_encode($dealer_bank->name) !!}+'">'+{!! json_encode($dealer_bank->name) !!}+'</option>'
                  }
                @endforeach
                html += '</select>'
                html += '</div>'
          ...
          let html = '<div class="col-sm-6">'
                ...
                html += '<div class="col-sm-6">'
                html += '<label class="form-label" for="bank-name">Bank Name</label>'
                html += '<select class="form-select" id="bank-name" name="bank_names[]">'
                @foreach ($banks as $buyer_bank)
                  html += '<option value="'+{!! json_encode($buyer_bank->name) !!}+'">'+{!! json_encode($buyer_bank->name) !!}+'</option>'
                @endforeach
                html += '</select>'
                html += '</div>'
                html += '<div class="col-sm-6">'
                ...
                html += '<div class="col-sm-6">'
                html += '<label class="form-label" for="swift-code">SWIFT Code</label>'
                html += '<input type="text" id="swift-code" class="form-control" name="swift_codes[]" />'
                html += '</div>'
                html += '<div class="col-sm-6">'
                html += '<label class="form-label" for="account-type">Account Type</label>'
                html += '<input type="text" id="account-type" class="form-control" name="account_types[]" />'
...
@section('content')
<h4 class="mb-2">
  <span class="fw-light">{{ __('Map Dealer to Program')}} - <span class="text-primary text-decoration-underline">{{ $program->name }}</span></span>
</h4>
<div class="">
  @if($errors->any())
  ...
  <div class="col-12 mb-4">
    <div class="bs-stepper wizard-vertical vertical mt-2" id="vendor-details-wizard">
      ...
      <div class="bs-stepper-content">
        <form id="vendor-details-form" method="POST" action="{{ route('programs.dealers.map.store', ['bank' => $bank, 'program' => $program]) }}">
          @csrf
          <!-- Company Details -->
          <div id="vendor-details" class="content">
            <div class="row g-3">
              <div class="col-sm-6">
                ...
                <select class="form-select select2" id="dealer" name="dealer_id" required>
                  <option value="">{{ __('Select')}}</option>
                  @foreach ($companies as $company)
                    <option value="{{ $company->id }}" @if(old('dealer_id') == $company->id) selected @endif>{{ $company->name }}</option>
                  @endforeach
                </select>
                <x-input-error :messages="$errors->get('dealer_id')" />
              </div>
              <div class="col-sm-6">
                <label for="payment-od-account">{{ __('Payment / OD Account')}} <span class="text-danger">*</span></label>
                <input type="text" id="payment-od-account" class="form-control" name="payment_account_number" value="{{ old('payment_account_number') }}" required />
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="invoice_margin" class="form-control" name="invoice_margin" disabled readonly value="{{ 100 - $program->eligibility }}" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="sanctioned_limit">{{ __('Sanctioned Limit')}} <span class="text-danger">*</span></label>
                <input type="text" id="sanctioned_limit" class="form-control" name="sanctioned_limit" max="{{ $sanctioned_limit }}" required />
              ...
              <div class="col-sm-6">
                <label class="form-label" for="limit_approved_date">
                  {{ __('Limit Approval Date')}}
              ...
              <div class="col-sm-6">
                <label class="form-label" for="limit-expiry-date">
                  {{ __('Limit Expiry Date')}}
                  <span class="text-danger">*</span>
                </label>
                <input class="form-control" type="date" id="limit_expiry_date" name="limit_expiry_date" value="{{ old('limit_expiry_date', Carbon\Carbon::parse($program->limit_expiry_date)->format('Y-m-d')) }}" required />
                <x-input-error :messages="$errors->get('limit_expiry_date')" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="limit_review_date">
                  {{ __('Limit Review Date')}}
                </label>
              ...
              <div class="col-sm-6">
                <label class="form-label" for="gst-no">
                  {{ __('KRA PIN') }}
                  <span class="text-danger">*</span>
                </label>
              ...
              <div class="col-12 d-flex justify-content-between">
                ...
                <div class="d-flex">
                  <a href="{{ route('programs.vendors.manage', ['bank' => $bank, 'program' => $program]) }}" class="btn btn-outline-danger">{{ __('Cancel') }}</a>
          ...
          <div id="discount-details" class="content">
            <div class="row g-3">
              ...
              <div class="col-sm-6">
                ...
                <select class="form-select" id="dealer-benchmark-title" disabled name="benchmark_title" onchange="changeDiscountRates()">
              ...
              <div class="col-sm-6">
                ...
                <input type="number" readonly id="current-dealer-benchmark-rate" class="form-control" min="0" max="100" step=".01" readonly value="{{ $program->discountDetails?->first()?->benchmark_rate }}" name="benchmark_rate" />
              ...
              <div class="col-12 row" id="program-discounts">
                @foreach ($program->dealerDiscountRates as $key => $discount_details)
                  @if ($loop->first)
                    <div class="col-sm-6">
                      <label class="form-label" for="from-day-{{ $key }}">
                        <span class="text-danger">*</span>
                        {{ __('From Day')}}
                      </label>
                      <input type="number" id="from-day-{{ $key }}" class="form-control" name="from_day[{{ $key }}]" @if($loop->first) readonly @endif value="{{ $discount_details->from_day }}" required />
                    </div>
                    <div class="col-sm-6">
                      <label class="form-label" for="to-day-{{ $key }}">
                        {{ __('To Day')}}
                        <span class="text-danger">*</span>
                      </label>
                      <input type="number" id="to-day-{{ $key }}" class="form-control" name="to_day[{{ $key }}]" value="{{ $discount_details->to_day }}" required />
                    </div>
                    <div class="col-sm-3">
                      <label class="form-label" for="business-strategy-spread-{{ $key }}">
                        {{ __('Business Strategy Spread')}} (%)
                        <span class="text-danger">*</span>
                      </label>
                      <input type="number" id="business-strategy-spread-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_business_strategy_spread[{{ $key }}]" required readonly value="{{ $discount_details->business_strategy_spread }}" />
                    </div>
                    <div class="col-sm-3">
                      <label class="form-label" for="credit-spread-{{ $key }}">{{ __('Credit Spread')}} (%)<span class="text-danger">*</span></label>
                      <input type="number" id="credit-spread-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_credit_spread[{{ $key }}]" required readonly value="{{ $discount_details->credit_spread }}" />
                    </div>
                    <div class="col-sm-3">
                      <label class="form-label" for="total-spread-{{ $key }}">{{ __('Total Spread')}} (%)<span class="text-danger">*</span></label>
                      <input type="number" id="total-spread-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_total_spread[{{ $key }}]" readonly value="{{ $discount_details->total_spread }}" required />
                    </div>
                    <div class="col-sm-3">
                      <label class="form-label" for="total-roi-{{ $key }}">{{ __('Total ROI')}} (%)<span class="text-danger">*</span></label>
                      <input type="number" id="total-roi-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_total_roi[{{ $key }}]" readonly value="{{ $discount_details->total_roi }}" required />
                    </div>
                    {{-- Discount Charge --}}
                    <div class="col-sm-3">
                  ...
                  @else
                    ...
                    <div class="col-sm-3" id="business-strategy-spread-section-{{ $key }}">
                      ...
                      <input type="number" id="business-strategy-spread-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_business_strategy_spread[{{ $key }}]" readonly value="{{ $discount_details->business_strategy_spread }}" />
                    </div>
                    <div class="col-sm-3" id="credit-spread-section-{{ $key }}">
                      <label class="form-label" for="credit-spread-{{ $key }}">{{ __('Credit Spread')}} (%)</label>
                      <input type="number" id="credit-spread-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_credit_spread[{{ $key }}]" readonly value="{{ $discount_details->credit_spread }}" />
                    </div>
                    <div class="col-sm-3" id="total-spread-section-{{ $key }}">
                      <label class="form-label" for="total-spread-{{ $key }}">{{ __('Total Spread')}} (%)</label>
                      <input type="number" id="total-spread-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_total_spread[{{ $key }}]" readonly value="{{ $discount_details->total_spread }}" />
                    </div>
                    <div class="col-sm-3" id="total-roi-section-{{ $key }}">
                      <label class="form-label" for="total-roi-{{ $key }}">{{ __('Total ROI')}} (%)</label>
                      <input type="number" id="total-roi-{{ $key }}" class="form-control" min="0" max="100" step=".01" name="dealer_total_roi[{{ $key }}]" readonly value="{{ $discount_details->total_roi }}" />
                    </div>
                    {{-- Discount Charge --}}
                    <div class="col-sm-3">
                      <label class="form-label" for="daily-discount-charge-{{ $key }}">
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="penal-discount-on-principle" class="form-control" min="0" max="100" step=".01" name="penal_discount_on_principle" readonly value="{{ $program->discountDetails?->first()?->penal_discount_on_principle }}" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="grace-period">{{ __('Grace Period (Days)')}}</label>
                <input type="number" id="grace-period" class="form-control" name="grace_period" readonly value="{{ $program->discountDetails?->first()?->grace_period }}" />
              </div>
              <div class="col-sm-6">
                <label class="form-label" for="grace-period-discount">{{ __('Grace Period Discount')}}</label>
                <input class="form-control" type="number" id="grace-period-discount" name="grace_period_discount" readonly value="{{ $program->discountDetails?->first()?->grace_period_discount }}" />
              </div>
              <div class="col-sm-6">
              ...
              <div class="col-sm-6">
                ...
                <input type="number" id="discount-posted" class="form-control" min="0" max="100" step=".0001" name="discount_posted" readonly value="{{ $program->discountDetails?->first()?->discount_on_posted_discount }}" />
              ...
              <div class="col-12 row" id="program-fees">
                @foreach ($program->fees as $key => $fee)
                  ...
                  <div class="col-sm-4 @if ($fee->type != 'per amount') d-none @endif" id="fee-per-amount-value-{{ $key }}">
                    ...
                    <input type="number" step=".00000001" id="fee-per-amount-value-input-{{ $key }}" class="form-control" name="fee_per_amount[{{ $key }}]" value="{{ $fee->per_amount }}" readonly />
                  </div>
                  <div class="col-sm-4">
                    <label class="form-label" for="value">{{ __('Value')}}</label>
                    <input type="number" step=".00000001" id="dealer-fee-value-{{ $key }}" class="form-control" name="fee_values[{{ $key }}]" value="{{ $fee->value }}" readonly oninput="updateDealerBearingFee({{ $key }})" />
                  </div>
                  <div class="col-sm-4">
                    <label class="form-label" for="dealer-fee-bearing">{{ __('Dealer Bearing')}}</label>
                    <input type="number" step=".00000001" id="dealer-fee-bearing-{{ $key }}" class="form-control" min="0" max="100" name="fee_dealer_bearing_discount[{{ $key }}]" value="{{ $fee->dealer_bearing }}" readonly />
                  </div>
          ...
          <div id="bank-details" class="content">
            <small class="text-danger">* First Account Entered is the default account</small>
