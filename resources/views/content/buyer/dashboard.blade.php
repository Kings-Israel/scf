...
<script>
  ...
  // Closed Invoiced
  // --------------------------------------------------------------------
  const earningReportsTabsClosedEl = document.querySelector('#earningReportsTabsClosed'),
    earningReportsTabsClosedConfig = EarningReportsBarChart(
      earningReportsChart['closed_payment_requests_data']
    );
  if (typeof earningReportsTabsClosedEl !== undefined && earningReportsTabsClosedEl !== null) {
    const earningReportsTabsClosed = new ApexCharts(earningReportsTabsClosedEl, earningReportsTabsClosedConfig);
    earningReportsTabsClosed.render();
  }
  ...
  document.getElementById('total-available-limit').innerHTML = new Intl.NumberFormat().format(earningReportsChart.total_available_limit);
  document.getElementById('total-utilized-limit').innerHTML = new Intl.NumberFormat().format(earningReportsChart.total_utilized_amount);
  document.getElementById('total-pipeline-limit').innerHTML = new Intl.NumberFormat().format(earningReportsChart.total_pipeline_amount);
  document.getElementById('total-overdue-amount').innerHTML = new Intl.NumberFormat().format(earningReportsChart.total_overdue_amount);
...
@section('content')
<h4 class="fw-bold">
  <span class="fw-light">{{ __('Buyer Dashboard')}}</span>
</h4>
    ...
    <div class="card">
      <div class="card-body border-bottom border-primary">
        <div class="d-flex mb-2">
          ...
          <h5 class="card-title px-2 my-2"><span id="total-program-limit">0</span></h5>
        </div>
    ...
    <div class="card">
      <div class="card-body border-bottom border-warning">
        <div class="d-flex mb-2">
          ...
          <h5 class="card-title px-2 my-2"><span id="total-available-limit">0</span></h5>
        </div>
    ...
    <div class="card">
      <div class="card-body border-bottom border-info">
        <div class="d-flex mb-2">
          ...
          <h5 class="card-title px-2 my-2"><span id="total-utilized-limit">0</span></h5>
        </div>
    ...
    <div class="card">
      <div class="card-body border-bottom border-warning">
        <div class="d-flex mb-2">
          ...
          <h5 class="card-title px-2 my-2"><span id="total-pipeline-limit">0</span></h5>
        </div>
    ...
    <div class="card">
      <div class="card-body border-bottom border-success">
        <div class="d-flex mb-2">
          ...
          <h5 class="card-title px-2 my-2"><span id="total-overdue-amount">0</span></h5>
        </div>
    ...
    <div class="card h-100">
      ...
      <div class="card-body">
        <ul class="nav nav-tabs widget-nav-tabs pb-3 gap-4 mx-1 d-flex flex-nowrap" role="tablist">
          ...
          <li class="nav-item">
            <a href="javascript:void(0);" class="nav-link btn d-flex flex-column align-items-center justify-content-center" role="tab" data-bs-toggle="tab" data-bs-target="#navs-closed-id" aria-controls="navs-closed-id" aria-selected="false">
              <h6 class="tab-widget-title mb-0 mt-2">{{ __('Closed Invoices')}}</h6>
            </a>
          </li>
        </ul>
        <div class="tab-content p-0 ms-0 ms-sm-2">
          ...
          <div class="tab-pane fade" id="navs-closed-id" role="tabpanel">
            <div id="earningReportsTabsClosed"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="col-12 mt-4">
      <div class="card">
        <div class="d-flex justify-content-between">
          <h5 class="card-header">{{ __('Summary of Vendors')}}</h5>
        </div>
            ...
            <tbody class="table-border-bottom-0 text-nowrap">
              @foreach ($vendors as $vendor)
                <tr>
                  ...
                  <td class="text-success">{{ number_format($vendor->overdue_amount) }}</td>
                  <td class="text-success">{{ number_format($vendor->total_requested_amount) }}</td>
                </tr>
              @endforeach
            </tbody>
          </table>
  ...
  <div class="col-12 mt-4">
    <div class="card">
      <div class="d-flex justify-content-between">
        ...
        <a class="card-header" href="{{ route('buyer.invoices.index') }}">{{ __('View All')}}</a>
      </div>
      <div class="table-responsive">
        <table class="table">
          <thead>
          ...
          <tbody class="table-border-bottom-0 text-nowrap">
            @foreach ($latest_invoices as $invoice)
              <tr>
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="modalCenterTitle">{{ __('Invoice')}}</h5>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                          <div class="d-flex">
                            <button class="btn btn-primary mx-2"> <i class="ti ti-printer"></i> {{ __('Print')}}</button>
                            <button class="btn btn-secondary"> <i class="ti ti-paperclip"></i> {{ __('Attachment')}}</button>
                          </div>
                        </div>
                            ...
                            <div class="mb-3">
                              @if ($invoice->pi_number)
                                <span class="d-flex justify-content-between">
                                  ...
                                  <h6 class="fw-bold mx-2 my-auto">{{ $invoice->pi_number }}</h6>
                                </span>
                              @endif
                              <span class="d-flex justify-content-between">
                ...
                <td class="text-success">{{ $invoice->currency }}. {{ number_format($invoice->overdue_amount) }}</td>
