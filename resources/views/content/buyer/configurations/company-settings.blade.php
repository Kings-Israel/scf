  ...
  <div class="col-md-6 col-lg-4 mb-3">
    <div class="card">
      <div class="card-body">
        ...
        <h5><strong>{{ __('Customer Type')}}: </strong><span class="fw-light">{{ Str::title($company->customer_type) }}</span></h5>
      </div>
    </div>
              ...
              <div class="accordion-item card">
                <div class="accordion-header p-1 d-flex justify-content-between">
                  <h2 class="">
                    <button type="button" class="accordion-button collapsed px-2 py-0" data-bs-toggle="collapse" data-bs-target="#accordionStyle1-{{ $key }}" aria-expanded="false">
                      {{ $document->name }}
                      <span class="badge {{ $document->resolveStatus() }} rounded-lg mx-1">{{ Str::title($document->status) }}</span> @if ($document->expiry_date)<span class="badge bg-label-success mx-1">{{ __('Expires in')}} {{ Carbon\Carbon::parse($document->expiry_date)->format('Y') }}</span> @endif
                    </button>
                  </h2>
                </div>
                <div id="accordionStyle1-{{ $key }}" class="accordion-collapse collapse" data-bs-parent="#accordionStyle1">
