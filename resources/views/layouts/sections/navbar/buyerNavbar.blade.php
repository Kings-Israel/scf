...
$active_company = \App\Models\Company::find(Auth::user()->activeBuyerCompany()->first()->company_id);
  ...
  <nav class="layout-navbar navbar navbar-expand-xl align-items-center bg-navbar-theme" id="layout-navbar">
    <div class="{{$containerNav}}">
      ...
      <div class="navbar-brand app-brand demo d-xl-flex py-0 me-4">
        <a href="{{url('/dealer/dashboard')}}" class="app-brand-link gap-2">
        ...
        <ul class="navbar-nav flex-row align-items-center ms-auto">
          <li class="nav-item">
            <div class="">
              <select id="defaultSelect" class="form-select mode-switcher">
                ...
                <option value="reverse-factoring">{{ __('Dealer') }}</option>
                @endif
                @if (auth()->user()->companies()->buyerFactoring()->count() > 0)
                <option value="factoring">{{ __('Buyer') }}</option>
                @endif
              </select>
            </div>
          </li>
          ...
          <li class="nav-item mx-2">
            <div class="">
              ...
              @if (!auth()->user()->activeBuyerCompany)
                @php(auth()->user()->activeBuyerCompany()->create(['company_id' => $companies->first()->id, 'platform' => 'buyer']))
              @endif
              @php($current_company = auth()->user()->activeBuyerCompany->company_id)
              <form action="{{ route('dealer.company.switch') }}" method="post" id="buyer-company-switch-form">
                @csrf
          ...
          <li class="nav-item me-2 me-xl-0">
            <a class="nav-link hide-arrow" href="{{ route('dealer.help.index') }}" target="_blank">
          ...
          <li class="nav-item dropdown-notifications navbar-dropdown dropdown me-3 me-xl-1">
            <a class="nav-link hide-arrow" href="{{ route('dealer.notifications') }}">
          ...
          <li class="nav-item navbar-dropdown dropdown-user dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
              <li>
                <a class="dropdown-item" href="{{ route('dealer.company.profile', ['company' => $active_company]) }}">
                  <div class="d-flex">
