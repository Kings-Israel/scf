...
$active_company = \App\Models\Company::find(Auth::user()->activeFactoringCompany()->first()->company_id);
    ...
    <div class="{{$containerNav}}">
      ...
      @if(isset($navbarFull))
        <div class="navbar-brand app-brand demo d-xl-flex py-0 me-4">
          <a href="{{url('/anchor/factoring')}}" class="app-brand-link gap-2">
      ...
      <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
        ...
        <ul class="navbar-nav flex-row align-items-center ms-auto">
          <li class="nav-item">
            <div class="">
              <select id="defaultSelect" class="form-select view-switcher">
                ...
                @endif
                @if (auth()->user()->has_any_vendor_financing_permissions && auth()->user()->companies()->anchor()->count() > 0)
          ...
          <li class="nav-item mx-2">
            <div class="">
              {{-- @php($companies = auth()->user()->companies()->anchorFactoring()->get()->merge(auth()->user()->companies()->dealerFinancing()->get())->unique()) --}}
              @php($companies = auth()->user()->companies()->where('status', 'active')->get())
              @if (!auth()->user()->activeFactoringCompany)
                @php(auth()->user()->activeFactoringCompany()->create(['company_id' => $companies->first()->id]))
              @endif
              @php($current_company = auth()->user()->activeFactoringCompany->company_id)
              <form action="{{ route('anchor.factoring.company.switch') }}" method="post" id="company-switch-form">
                @csrf
          ...
          <li class="nav-item me-2 me-xl-0">
            <a class="nav-link hide-arrow" href="{{ route('anchor.factoring-help-index') }}" target="_blank">
          ...
          <li class="nav-item dropdown-notifications navbar-dropdown dropdown me-3 me-xl-1">
            <a class="nav-link hide-arrow" href="{{ route('anchor.factoring-notifications') }}">
          ...
          <li class="nav-item navbar-dropdown dropdown-user dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
              <li>
                <a class="dropdown-item" href="{{ route('anchor.factoring-company.profile', ['company' => $active_company]) }}">
                  <div class="d-flex">
